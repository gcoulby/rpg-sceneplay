...
import {
  ...
  selectForPruning,
  type ParsedBackup,
} from '../utils/open-draft/backupNaming'
import {
  serializeOdraft,
  parseOdraft,
  parseOdraftLoose,
...
export async function writeSnapshot(
  ...
  const { backupFolder, backupIncludeImages, backupRetentionCount } =
  ...
  for (const e of entries) {
    ...
    if (parsed) {
      ...
      out.push({
        path: e.path,
        name: e.name,
        title: e.name.replace(/\.odraft$/i, ''),
        scriptKey: 'external',
        date: new Date(e.modified_ms || 0),
        kind: 'external',
        sizeBytes: e.size,
        project: e.project,
      })
...
export async function pruneSnapshots(keep?: number): Promise<number> {
  ...
  const doomed = selectForPruning(parsed, limit) as Array<
    ParsedBackup & { path: string }
  >
  let removed = 0
