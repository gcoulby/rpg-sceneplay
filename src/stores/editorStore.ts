      ...
      if (parsed && typeof parsed === 'object') {
        ...
        for (const [name, words] of Object.entries(parsed)) {
          if (Array.isArray(words))
            out[name] = words.filter((w): w is string => typeof w === 'string')
        }
        return out
      }
    }
    ...
    if (legacy) {
      try {
        ...
        if (Array.isArray(arr)) {
          const migrated = {
            Personal: arr.filter((w): w is string => typeof w === 'string'),
          }
...
function saveAddTargets(targets: string[]) {
  try {
    localStorage.setItem(ADD_TARGETS_KEY, JSON.stringify(targets))
  } catch {
    /* noop */
  }
}
...
function saveInstalledLanguages(codes: string[]) {
  try {
    localStorage.setItem(INSTALLED_LANGS_KEY, JSON.stringify(codes))
  } catch {
    /* noop */
  }
}
...
export function resolveMoresContds(
  layout: PageLayout | undefined,
): MoresContds {
  const m = layout?.moresContds
  const resolved: MoresContds = { ...DEFAULT_MORES_CONTDS, ...(m ?? {}) }
  if (!resolved.contdText.trim())
    resolved.contdText = DEFAULT_MORES_CONTDS.contdText
  if (!resolved.moreText.trim())
    resolved.moreText = DEFAULT_MORES_CONTDS.moreText
  return resolved
}
...
export const DEFAULT_PAGE_LAYOUT: PageLayout = {
  ...
  headerMargin: 36,
  footerMargin: 36,
  leftMargin: 1.5, // Final Draft default LeftIndent for Action
...
interface EditorState {
  ...
  updateNote: (
    id: string,
    updates: Partial<Pick<NoteInfo, 'content' | 'color'>>,
  ) => void
  ...
  updateGeneralNote: (
    id: string,
    updates: Partial<Pick<GeneralNote, 'title' | 'content' | 'color'>>,
  ) => void
  ...
  updateBeatColumn: (
    id: string,
    updates: Partial<{ title: string; position: number; width: number }>,
  ) => void
  ...
  addTag: (
    tag: Omit<TagItem, 'id' | 'createdAt' | 'name'> & { name?: string },
  ) => string
  updateTag: (
    id: string,
    updates: Partial<Pick<TagItem, 'notes' | 'categoryId' | 'name'>>,
  ) => void
  ...
  setCharacterSortBy: (
    sort: 'name' | 'importance' | 'scenes' | 'dialogues' | 'appearance',
  ) => void
  ...
  setSpellingSetting: <K extends keyof SpellingSettings>(
    key: K,
    value: Required<SpellingSettings>[K],
  ) => void
  ...
  // ── Action-guard state (used by actions/shared.ts + the New Screenplay
  //    format-selection flow). Lives here, not as component-local state, so
  //    that plain functions in the actions/ folder can drive these dialogs
  //    without needing a React component to own them. ──
  /** Blocks a destructive action (New / Open / Import) behind a Save/Discard
   *  prompt when there's unsaved work. Set by confirmOrRun(); consumed by
   *  the DiscardConfirmDialog. */
  discardConfirmOpen: boolean
  setDiscardConfirmOpen: (open: boolean) => void
  /** The action deferred behind discardConfirmOpen — run on Discard, or after
   *  a successful Save. */
  pendingAction: (() => void) | null
  setPendingAction: (action: (() => void) | null) => void
  /** Multi-select "which script formats do you use" dialog. `firstRun` makes
   *  it non-cancellable; `afterSave` tells the caller what to resume once the
   *  user confirms (currently only the New Screenplay flow uses this). */
  formatPrefsOpen: {
    firstRun: boolean
    afterSave: 'apply-new-screenplay' | null
  } | null
  setFormatPrefsOpen: (
    v: { firstRun: boolean; afterSave: 'apply-new-screenplay' | null } | null,
  ) => void
  /** Single-select format picker, shown when 2+ formats are enabled. */
  formatPickerOpen: boolean
  setFormatPickerOpen: (open: boolean) => void
  /** 'reset' = top-level New Screenplay (clears project context). 'apply-only'
   *  = in-project script creation (ProjectView already set project context;
   *  don't wipe it). */
...
function _pushBeatSnapshot(get: () => EditorState, force = false) {
  const s = get() as EditorState & {
    _beatIsUndoing: boolean
    _beatUndoStack: unknown[]
    _beatSnapshotTime: number
  }
  if (s._beatIsUndoing) {
  ...
  const stack = [
    ...(s._beatUndoStack as { beats: BeatInfo[]; beatColumns: BeatColumn[] }[]),
    { beats: s.beats, beatColumns: s.beatColumns },
  ]
  if (stack.length > BEAT_UNDO_MAX) stack.shift()
  useEditorStore.setState({
    _beatUndoStack: stack,
    _beatRedoStack: [],
    _beatSnapshotTime: now,
    canBeatUndo: true,
    canBeatRedo: false,
  } as any)
}
...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  updateGeneralNote: (id, updates) =>
    set((s) => ({
      generalNotes: s.generalNotes.map((n) =>
        n.id === id ? { ...n, ...updates } : n,
      ),
    })),
  ...
  beatUndo: () =>
    set((s) => {
      const stack = s._beatUndoStack as {
        beats: BeatInfo[]
        beatColumns: BeatColumn[]
      }[]
  ...
  beatRedo: () =>
    set((s) => {
      const stack = s._beatRedoStack as {
        beats: BeatInfo[]
        beatColumns: BeatColumn[]
      }[]
      if (stack.length === 0) return {}
  ...
  addBeatColumn: (title) => {
    ...
    set((s) => {
      const maxPos =
        s.beatColumns.length > 0
          ? Math.max(...s.beatColumns.map((c) => c.position))
          : -1
      return {
        beatColumns: [
          ...s.beatColumns,
          { id, title, position: maxPos + 1, width: 0 },
        ],
      }
    })
    return id
  ...
  updateBeatColumn: (id, updates) => {
    _pushBeatSnapshot(get)
    set((s) => ({
      beatColumns: s.beatColumns.map((c) =>
        c.id === id ? { ...c, ...updates } : c,
      ),
    }))
  },
  ...
  addBeat: (title, columnId) => {
    ...
    set((s) => {
      ...
      const maxPos =
        colBeats.length > 0 ? Math.max(...colBeats.map((b) => b.position)) : -1
      return {
        beats: [
          ...s.beats,
          {
  ...
  deleteCharacterProfile: (name) =>
    set((s) => ({
      characterProfiles: s.characterProfiles.filter(
  ...
  deleteCharacterRelationship: (id) =>
    set((s) => ({
      characterRelationships: s.characterRelationships.filter(
        (r) => r.id !== id,
      ),
    })),
  ...
  setZoomLevel: (level) => {
    const clamped = Math.min(300, Math.max(50, level))
    set({ zoomLevel: clamped })
    saveViewState({ zoomLevel: clamped })
  },
  zoomPanelOpen: false,
  ...
  toolbarMode:
    (_vs.toolbarMode as 'compact' | 'comfortable' | 'hidden') ?? 'compact',
  setToolbarMode: (mode) => {
    set({ toolbarMode: mode })
    saveViewState({ toolbarMode: mode })
  },

  characterSortBy:
    (_vs.characterSortBy as
      | 'name'
      | 'importance'
      | 'scenes'
      | 'dialogues'
      | 'appearance') ?? 'importance',
  setCharacterSortBy: (sort) => {
    set({ characterSortBy: sort })
    saveViewState({ characterSortBy: sort })
  },
  ...
  toggleGrammarCheck: () =>
    set((s) => ({ grammarCheckEnabled: !s.grammarCheckEnabled })),
  ...
  grammarRulesEnabled:
    (_vs.grammarRulesEnabled as Record<string, boolean> | undefined) ?? {},
  setGrammarRuleEnabled: (ruleId, enabled) =>
    set((s) => {
  ...
  spellingSettings: {
    ...DEFAULT_SPELLING_SETTINGS,
    ...(_vs.spellingSettings ?? {}),
  },
  setSpellingSetting: (key, value) =>
    set((s) => {
  ...
  renameGlobalDictionary: (oldName, newName) =>
    set((s) => {
      ...
      if (enabled.includes(oldName)) {
        spellChecker.setEnabledGlobalDicts(
          enabled.map((n) => (n === oldName ? trimmed : n)),
        )
      }
      return { customDictionaries: next }
    }),
  ...
  setGlobalDictionaryWords: (name, words) =>
    set((s) => {
      if (!(name in s.customDictionaries)) return s
  ...
  setAddTargets: (targets) =>
    set(() => {
      ...
      const next =
        targets.length > 0 ? [...new Set(targets)] : [PROJECT_DICT_TARGET]
      saveAddTargets(next)
      spellChecker.setAddTargets(next)
      return { addTargets: next }
    }),
  appendWordToGlobalDictionary: (name, word) =>
    set((s) => {
      if (!(name in s.customDictionaries)) return s
  ...
  installLanguage: async (code) => {
    ...
    if (!ok) {
      return {
        ok: false,
        error: `Couldn't download "${lang.label}" — the source may be temporarily unavailable, or the network is offline.`,
      }
    }
    set((s) => {
  ...
  installLanguageFromUrls: async ({ code, label, affUrl, dicUrl }) => {
    ...
    if (!ok) {
      return {
        ok: false,
        error:
          'Download or parse failed. Check the URLs and that the files are valid Hunspell .aff/.dic.',
      }
    }
    set((s) => {
  ...
  setPendingFormatPromptInProject: (v) =>
    set({ pendingFormatPromptInProject: v }),
  postSaveAction: null,
  ...
  setDiscardConfirmOpen: (open) => set({ discardConfirmOpen: open }),
  pendingAction: null,
  setPendingAction: (action) => set({ pendingAction: action }),
  formatPrefsOpen: null,
  setFormatPrefsOpen: (v) => set({ formatPrefsOpen: v }),
