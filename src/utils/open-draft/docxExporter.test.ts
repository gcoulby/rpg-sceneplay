...
function hasElement(node: unknown, rootKey: string): boolean {
  ...
  const children = Array.isArray(n.root)
    ? n.root
    : n.root !== undefined
      ? [n.root]
      : []
...
const countBreaks = (runs: unknown[]) =>
  runs.filter((r) => hasElement(r, 'w:br')).length
...
describe('DOCX buildTextRuns with hard breaks', () => {
  ...
  it('does not filter the break out as an empty run', () => {
    expect(
      countBreaks(buildTextRuns(jsonBlockRuns(block('action', 'a', BR, 'b')))),
    ).toBe(1)
  })
  ...
  it('emits one break per consecutive hard break', () => {
    expect(
      countBreaks(
        buildTextRuns(jsonBlockRuns(block('action', 'a', BR, BR, 'b'))),
      ),
    ).toBe(2)
  })
