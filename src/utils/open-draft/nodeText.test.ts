...
describe('jsonBlockText', () => {
  it('joins text runs', () => {
    expect(jsonBlockText(block('action', 'Hello ', 'world'))).toBe(
      'Hello world',
    )
  })

  it('renders a hard break as a newline instead of dropping it', () => {
    expect(jsonBlockText(block('action', 'Line one', BR, 'Line two'))).toBe(
      'Line one\nLine two',
    )
  })
  ...
  it('ignores leaves that are neither text nor breaks', () => {
    expect(
      jsonBlockText({
        type: 'action',
        content: [{ type: 'screenplayImage' }, { type: 'text', text: 'x' }],
      }),
    ).toBe('x')
  })
})
...
describe('jsonBlockLines', () => {
  it('splits on breaks', () => {
    expect(jsonBlockLines(block('action', 'a', BR, 'b', BR, 'c'))).toEqual([
      'a',
      'b',
      'c',
    ])
  ...
  it('yields an empty segment for a double break', () => {
    expect(jsonBlockLines(block('action', 'a', BR, BR, 'b'))).toEqual([
      'a',
      '',
      'b',
    ])
...
describe('jsonBlockRuns', () => {
  ...
  it('flags break runs and keeps them in position', () => {
    ...
    expect(runs.map((r) => [r.text, r.isBreak])).toEqual([
      ['a', false],
      ['', true],
      ['b', false],
    ])
...
describe('blockContentRange', () => {
  it.each([0, 1, 2])(
    'spans the whole inline content with %i breaks',
    (breaks) => {
  ...
  it('is correct for a non-zero node position', () => {
    const d = pmDoc(
      doc(block('action', 'first'), block('action', 'x', BR, 'y')),
    )
