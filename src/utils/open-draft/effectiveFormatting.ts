...
export function getEffectiveFormatting(
  ...
  if (!isOverride) {
    ...
    return {
      isBold: locked.bold
        ? templateBold
        : templateBold || editor.isActive('bold'),
      isItalic: locked.italic
        ? templateItalic
        : templateItalic || editor.isActive('italic'),
      isUnderline: locked.underline
        ? templateUnderline
        : templateUnderline || editor.isActive('underline'),
      isStrikethrough: locked.strikethrough
        ? templateStrike
        : templateStrike || editor.isActive('strike'),
      textAlign: locked.textAlign
        ? templateAlign
        : editor.getAttributes('textAlign')?.textAlign || templateAlign,
    }
  }
 ...
 */
export function toggleBoldOverride(
  editor: Editor,
  rule: FormattingElementRule | null,
): void {
  if (rule?.bold) {
    ...
    if (attrs?.fontWeight === 'normal') {
      editor
        ...
        .unsetMark('formatOverride')
        .run()
    } else {
      editor
        .chain()
        .focus(undefined, { scrollIntoView: false })
        .setMark('formatOverride', { fontWeight: 'normal' })
        .run()
    }
  } else {
    editor
...
export function toggleItalicOverride(
  editor: Editor,
  rule: FormattingElementRule | null,
): void {
  if (rule?.italic) {
    ...
    if (attrs?.fontStyle === 'normal') {
      ...
      editor
        .chain()
        .focus(undefined, { scrollIntoView: false })
        .setMark('formatOverride', { fontStyle: 'normal' })
        .run()
    }
  } else {
    editor
...
export function toggleUnderlineOverride(
  editor: Editor,
  rule: FormattingElementRule | null,
): void {
  if (rule?.underline) {
    ...
    if (attrs?.textDecoration === 'none') {
      ...
      editor
        .chain()
        .focus(undefined, { scrollIntoView: false })
        .setMark('formatOverride', { textDecoration: 'none' })
        .run()
    }
  } else {
    editor
