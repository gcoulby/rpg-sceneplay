...
function isMarkLocked(markName: string, locked: LockedFormatting): boolean {
  switch (markName) {
    case 'bold':
      return locked.bold
    case 'italic':
      return locked.italic
    case 'underline':
      return locked.underline
    case 'strike':
      return locked.strikethrough
    case 'subscript':
      return locked.subscript
    case 'superscript':
      return locked.superscript
    case 'textStyle':
      return locked.fontFamily || locked.fontSize
    case 'color':
      return locked.textColor
    case 'highlight':
      return locked.backgroundColor
    case 'formatOverride':
      return true // always stale when switching templates
    default:
      return false
  }
}
...
export function detectTemplateConflicts(
  ...
  const violationMap = new Map<
    string,
    { label: string; marks: Set<string>; count: number }
  >()

  // Get enabled element options for default replacement validation
  const enabledIds = new Set(
    Object.values(template.rules)
      .filter((r) => r.enabled)
      .map((r) => r.id),
  )
  ...
  editor.state.doc.descendants((node, _pos) => {
    ...
    if (
      disabledSet.has(typeId) ||
      (!template.rules[typeId] && node.type.name !== 'doc')
    ) {
      // Element type is disabled or not in template at all
      const existing = disabledCounts.get(typeId)
      const label =
        template.rules[typeId]?.label ||
        (node.type.name === 'customElement'
          ? node.attrs?.customLabel || typeId
          : typeId)
      if (existing) {
        existing.count++
      } else {
  ...
  return {
    ...
    hasConflicts:
      disabledElements.length > 0 || formattingViolations.length > 0,
  }
}
  ...
  editor.state.doc.descendants((node, pos) => {
    ...
    if (needsReplace && needsReformat) {
      nodeOps.push({
        ...
        typeId,
        action: 'both',
        replacementType: replacementMap.get(typeId),
      })
  ...
  for (let i = nodeOps.length - 1; i >= 0; i--) {
    ...
    if (op.action === 'reformat' || op.action === 'both') {
      const effectiveTypeId =
        op.action === 'both' ? op.replacementType! : op.typeId
