...
function resolveFdxExportType(node: JSONContent): string {
  if (node.type === 'customElement') {
    const cid = (node.attrs as { customTypeId?: string } | undefined)
      ?.customTypeId
...
function buildElementSettings(lm: number, ri: number): string {
  ...
  const character_l = lm + 2.5,
    parenthetical_l = lm + 2.0,
    dialogue_l = lm + 1.31
  const transition_l = lm + 4.0,
    castList_l = lm + 0.25,
    outline2_l = lm + 0.5
  const dialogue_r = ri - 1.0,
    parenthetical_r = ri - 2.0,
    transition_r = ri - 0.5
  ...
  for (const mark of marks) {
    ...
    if (mark.type === 'textStyle' && mark.attrs) {
      ...
      if (mark.attrs.fontSize)
        fontSize = String(mark.attrs.fontSize).replace('pt', '')
...
export function exportFDX(
  ...
  lines.push(
    `  <PageLayout BackgroundColor="#FFFFFFFFFFFF" BottomMargin="${bm}" BreakDialogueAndActionAtSentences="Yes" DocumentLeading="Normal" FooterMargin="${fm}" ForegroundColor="#000000000000" HeaderMargin="${hm}" InvisiblesColor="#808080808080" TopMargin="${tm}" UsesSmartQuotes="No">`,
  )
  ...
  const rightIndent = pageLayout
    ? pageLayout.pageWidth - pageLayout.rightMargin
    : 7.25
  ...
  if (doc.content) {
    for (const node of doc.content) {
      if (
        ...
        tpDraft = [node.attrs.tpDraft, node.attrs.tpDraftDate]
          .filter(Boolean)
          .join(' - ')
  ...
  lines.push(
    `      <Paragraph Type="General" Alignment="Center" SpaceBefore="288"><Text>${esc(tpTitle)}</Text></Paragraph>`,
  )
  if (tpWrittenBy) {
    lines.push(
      `      <Paragraph Type="General" Alignment="Center"><Text>Written by</Text></Paragraph>`,
    )
    lines.push(
      `      <Paragraph Type="General" Alignment="Center"><Text>${esc(tpWrittenBy)}</Text></Paragraph>`,
    )
    if (tpBasedOn) {
      lines.push(
        `      <Paragraph Type="General" Alignment="Center"><Text>${esc(tpBasedOn)}</Text></Paragraph>`,
      )
    }
  }
  if (tpDraft) {
    lines.push(
      `      <Paragraph Type="General"><Text>${esc(tpDraft)}</Text></Paragraph>`,
    )
  }
  if (tpContact) {
    for (const line of tpContact.split('\n')) {
      lines.push(
        `      <Paragraph Type="General" Alignment="Right"><Text>${esc(line)}</Text></Paragraph>`,
      )
    }
  }
  if (tpCopyright) {
    lines.push(
      `      <Paragraph Type="General"><Text>${esc(tpCopyright)}</Text></Paragraph>`,
    )
  }
  lines.push('    </Content>')
  lines.push('  </TitlePage>')
  ...
  if (beats && beats.length > 0) {
    ...
    for (const col of sortedCols) {
      ...
      lines.push(
        `    <Paragraph Type="Outline 1"><Text>${esc(col.title)}</Text></Paragraph>`,
      )
      for (const beat of colBeats) {
        lines.push(
          `    <Paragraph Type="Outline 2"><Text>${esc(beat.title)}</Text></Paragraph>`,
        )
        if (beat.description) {
          for (const descLine of beat.description.split('\n')) {
            lines.push(
              `    <Paragraph Type="Outline Body"><Text>${esc(descLine)}</Text></Paragraph>`,
            )
          }
        }
      }
    ...
    for (const [colId, colBeats] of beatsByCol) {
      ...
      for (const beat of colBeats) {
        lines.push(
          `    <Paragraph Type="Outline 1"><Text>${esc(beat.title)}</Text></Paragraph>`,
        )
        if (beat.description) {
          for (const descLine of beat.description.split('\n')) {
            lines.push(
  ...
  const emitParagraph = (node: JSONContent, indent: string) => {
    ...
    if (node.attrs?.sceneNumber)
      paraAttrs.push(`Number="${node.attrs.sceneNumber}"`)
    ...
    if (node.content && node.content.length > 0) {
      ...
      if (node.type === 'sceneHeading' && node.attrs?.synopsis) {
        ...
        lines.push(
          `${indent}        <Text>${esc(String(node.attrs.synopsis))}</Text>`,
        )
        lines.push(`${indent}      </Paragraph>`)
        lines.push(`${indent}    </Summary>`)
        lines.push(`${indent}  </SceneProperties>`)
        ...
        if (node.content) {
          ...
          for (const row of node.content) {
            ...
            for (const cell of row.content) {
              ...
              const side =
                (cell.attrs as { side?: string } | undefined)?.side || 'video'
              for (const para of cell.content) {
                ...
                lines.push(
                  `    <Paragraph Type="General" data-av-side="${esc(side)}" data-av-row-id="${esc(rowId)}"><Text>${esc(text)}</Text></Paragraph>`,
                )
              }
            }
          }
  ...
  if (characterProfiles && characterProfiles.length > 0) {
    ...
    for (const p of characterProfiles) {
      if (p.color) {
        lines.push(
          `    <Character Name="${esc(p.name)}" Color="${esc(p.color)}" Highlighted="${p.highlighted ? 'Yes' : 'No'}"/>`,
        )
      }
    }
    lines.push('  </CharacterHighlighting>')
  ...
  if (tagCategories && tags && tags.length > 0) {
    ...
    for (const cat of usedCats) {
      lines.push(
        `      <TagCategory CatId="${esc(cat.id)}" Name="${esc(cat.name)}" Color="${esc(cat.color)}"/>`,
      )
    }
    lines.push('    </TagCategories>')
    lines.push('    <TagItems>')
    for (const tag of tags) {
      lines.push(
        `      <TagItem TagId="${esc(tag.id)}" CatId="${esc(tag.categoryId)}" Label="${esc(tag.name || tag.text)}"/>`,
      )
    }
    lines.push('    </TagItems>')
    lines.push('  </TagData>')
  ...
  if (beats && beats.length > 0) {
    ...
    lines.push(
      '    <DisplayBoard Height="55" ScrollOrigin="0,0" Type="StoryMap" Width="2032" ZoomLevel="100.000"/>',
    )
    lines.push(
      '    <DisplayBoard Height="10000" ScrollOrigin="0,0" Type="Beat" Width="24000" ZoomLevel="100.000"/>',
    )
    lines.push('  </DisplayBoards>')
  }
...
export async function downloadFDX(
  doc: JSONContent,
  title: string = 'Untitled',
  characterProfiles?: CharacterProfile[],
  tagCategories?: TagCategory[],
