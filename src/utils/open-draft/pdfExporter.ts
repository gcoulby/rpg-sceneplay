...
export async function exportPDF(
  ...
  if (!doc || !doc.content || doc.content.length === 0) {
    const pdf = new jsPDF({
      unit: 'pt',
      format: [
        layout.pageWidth * PTS_PER_INCH,
        layout.pageHeight * PTS_PER_INCH,
      ],
    })
    await saveFile(new Uint8Array(pdf.output('arraybuffer')), filename, [
  ...
  for (const node of doc.content) {
    ...
    if (
      inLeadingRegion &&
      (typeName === 'titlePage' || typeName === 'screenplayImage')
    ) {
  ...
  const titleImgData = new Map<
    number,
    { dataUrl: string; wPt: number; hPt: number }
  >()
  if (hasTitlePage) {
    const contentW =
      pageWidthPt - (layout.leftMargin + layout.rightMargin) * PTS_PER_INCH
    for (let k = 0; k < titleItems.length; k++) {
      ...
      let wPt =
        widthPx > 0 ? widthPx * 0.75 : Math.min(d.width * 0.75, contentW * 0.6)
      wPt = Math.min(wPt, contentW)
      titleImgData.set(k, {
        dataUrl: d.dataUrl,
        wPt,
        hPt: wPt * (d.height / (d.width || 1)),
      })
    }
  }
    ...
    for (let k = 0; k < titleItems.length; k++) {
      ...
      if (it.kind === 'image') {
        ...
        const align: 'left' | 'center' | 'right' =
          it.field === 'draft'
            ? 'left'
            : it.field === 'contact' || it.field === 'copyright'
              ? 'right'
              : 'center'
        ...
        for (const line of lines) {
          if (line && y + lineH <= bottom)
            pdf.text(isTitle ? line.toUpperCase() : line, x, y + lineH, {
              align,
            })
          y += lineH
        }
  ...
  const imageMap = new Map<
    number,
    { dataUrl: string; wPt: number; hPt: number; align: string }
  >()
  for (let k = 0; k < nodes.length; k++) {
    ...
    let wPt =
      widthPx > 0
        ? widthPx * 0.75
        : Math.min(d.width * 0.75, contentWidthPt * 0.9)
  ...
  while (i < nodes.length) {
    ...
    if (typeName === 'screenplayImage') {
      ...
      if (img) {
        ...
        if (
          currentY + sbPt + img.hPt > pageHeightPt - bottomMarginPt &&
          currentY > topMarginPt
        ) {
          newPage()
        } else {
        ...
        let x = contentLeft
        if (img.align === 'center')
          x = (contentLeft + contentRight) / 2 - img.wPt / 2
    ...
    if (typeName === 'character') {
      ...
      while (j < nodes.length && DIALOGUE_BLOCK_TYPES.has(nodes[j].typeName)) {
        ...
        const dLines = wordWrapRuns(
          dNode.runs,
          dMaxChars,
          UPPERCASE_TYPES.has(dNode.typeName),
        )
    ...
    if (typeName === 'sceneHeading' && i + 1 < nodes.length) {
      ...
      const nLines = wordWrapRuns(
        nNode.runs,
        nMaxChars,
        UPPERCASE_TYPES.has(nNode.typeName),
      )
    ...
    if (
      isDialogueBlock &&
      currentY + dialogueBlockHeight > usableBottomPt &&
      currentY > topMarginPt + LINE_HEIGHT_PT
    ) {
      ...
      if (remaining >= minSplitHeight) {
        ...
        if (dIdx < dialogueBlockNodes.length) {
          ...
          if (
            mc.dialogueBreakContd &&
            currentY + LINE_HEIGHT_PT <= usableBottomPt
          ) {
            setFontStyle(pdf, false, false)
            pdf.text(mc.moreText, moreLeftPt, currentY + LINE_HEIGHT_PT, {
              charSpace,
            })
          }
          ...
          while (dIdx < dialogueBlockNodes.length) {
            ...
            if (currentY + dHeight > usableBottomPt) {
              if (
                ...
                setFontStyle(pdf, false, false)
                pdf.text(mc.moreText, contdLeftPt, currentY + LINE_HEIGHT_PT, {
                  charSpace,
                })
              }
              newPage()
...
function renderHFLine(
  ...
  // Center
  const centerText = resolveFields(
    content.center,
    pageNum,
    totalPages,
    title,
