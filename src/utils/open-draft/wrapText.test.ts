...
import { getTextLines, wordWrapRuns } from '@/utils/open-draft/wrapText'
import { jsonBlockText, jsonBlockRuns } from '@/utils/open-draft/nodeText'
...
describe('getTextLines agrees with the PDF word wrapper', () => {
  ...
  const cases: Array<[string, ReturnType<typeof block>]> = [
    ...
    [
      'break between wrapped runs',
      block('action', wrappable.repeat(2), BR, wrappable.repeat(2)),
    ],
...
describe('wordWrapRuns with breaks', () => {
  ...
  it('starts a new line at a break', () => {
    expect(
      plain(
        wordWrapRuns(extractRuns(block('action', 'one', BR, 'two')), 60, false),
      ),
    ).toEqual(['one', 'two'])
  })
  ...
  it('produces a genuinely blank line for a double break', () => {
    expect(
      plain(
        wordWrapRuns(extractRuns(block('action', 'a', BR, BR, 'b')), 60, false),
      ),
    ).toEqual(['a', '', 'b'])
  })
  ...
  it('does not uppercase a break run', () => {
    const lines = wordWrapRuns(
      extractRuns(block('character', 'jo', BR, 'hn')),
      60,
      true,
    )
