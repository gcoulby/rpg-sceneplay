...
export function buildTextRuns(runs: RunStyle[]): TextRun[] {
  if (
    runs.length === 0 ||
    (runs.length === 1 && runs[0].text === '' && !runs[0].isBreak)
  ) {
    return [
...
function templateToChildren(
  ...
  const pushText = (txt: string) => {
    ...
    out.push(
      new TextRun({ text: txt, font: FONT_FAMILY, size: FONT_SIZE_HALFPT }),
    )
  }
...
function buildTitlePageFlow(
  ...
  images: Map<
    number,
    { data: Uint8Array; w: number; h: number; align: string }
  >,
): Paragraph[] {
  ...
  nodes.forEach((node, i) => {
    ...
    const align =
      field === 'draft'
        ? AlignmentType.LEFT
        : field === 'contact' || field === 'copyright'
          ? AlignmentType.RIGHT
          : AlignmentType.CENTER
    const size = isTitle
      ? (Number(node.attrs?.tpTitleFontSize) || 12) * 2
      : FONT_SIZE_HALFPT
...
export async function exportDocx(
  ...
  if (doc?.content) {
    for (const node of doc.content) {
      if (
        inLeadingRegion &&
        (node.type === 'titlePage' || node.type === 'screenplayImage')
      ) {
        if (
          node.type === 'titlePage' &&
  ...
  if (!hasTitlePage && titleRegionNodes.length > 0) {
    bodyNodes.unshift(
      ...titleRegionNodes.filter((n) => n.type === 'screenplayImage'),
    )
  ...
  const skipFirstPage =
    (layout.headerStartPage ?? 2) >= 2 || (layout.footerStartPage ?? 1) >= 2
  ...
  for (let i = 0; i < bodyNodes.length; i++) {
    ...
    let w =
      widthPx > 0
        ? widthPx
        : Math.min(b.width, Math.round(contentWidthPx * 0.9))
  ...
  for (let i = 0; i < titleRegionNodes.length; i++) {
    ...
    let w =
      widthPx > 0
        ? widthPx
        : Math.min(b.width, Math.round(contentWidthPx * 0.5))
    w = Math.min(w, Math.round(contentWidthPx))
    const h = Math.round(w * (b.height / (b.width || 1)))
    titleImageMap.set(i, {
      data: b.data,
