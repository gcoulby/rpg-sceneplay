...
export function generateTemplateCss(
  ...
  for (const [elementId, rule] of Object.entries(template.rules)) {
    ...
    if (rule.placeholder) {
      ...
      if (rule.textTransform === 'uppercase')
        pStyles.push('text-transform: uppercase;')
...
function generateRuleProperties(
  ...
  if (rule.fontFamily) {
    props.push(
      `font-family: '${rule.fontFamily}', 'Courier Prime', 'Courier New', monospace;`,
    )
  }
  ...
  if (rightPad > 0.01) {
    props.push(
      `padding-right: calc(var(--pw, ${pw}in) - ${rule.rightIndent}in - var(--pr, ${prMargin}in));`,
    )
  } else {
