...
export function computeCharacterDialogue(
  ...
  const characterMap = new Map<
    string,
    { lines: number; words: number; sceneSet: Set<number> }
  >()
  ...
  for (const [name, data] of characterMap) {
    ...
    stats.push({
      ...
      dialoguePercentage:
        totalDialogueWords > 0 ? (data.words / totalDialogueWords) * 100 : 0,
...
export function computeGenderBreakdown(
  ...
  const genderMap = new Map<
    string,
    { characters: number; lines: number; words: number }
  >()
  let totalWords = 0
...
export function computeSceneBreakdown(doc: JSONContent): SceneBreakdownStats {
  ...
  for (const scene of scenes) {
    ...
    if (
      scene.timeOfDay === 'DAY' ||
      scene.timeOfDay === 'MORNING' ||
      scene.timeOfDay === 'AFTERNOON'
    )
      dayCount++
    else if (scene.timeOfDay === 'NIGHT' || scene.timeOfDay === 'EVENING')
      nightCount++
    else otherTimeCount++
    ...
    if (scene.location) {
      locationCounts.set(
        scene.location,
        (locationCounts.get(scene.location) || 0) + 1,
      )
    }
  }
...
export function computePacingData(doc: JSONContent): PacingDataPoint[] {
  ...
  return scenes.map((scene, i) => {
    let dialogueWords = 0
    for (const data of scene.dialogueByCharacter.values())
      dialogueWords += data.words
    return {
      sceneIndex: i,
...
export function computeCharacterPresence(
  ...
  const allCharacters = new Map<
    string,
    { color: string; scenesPresent: Set<number> }
  >()
  ...
  const entries = Array.from(allCharacters.entries()).sort(
    (a, b) => b[1].scenesPresent.size - a[1].scenesPresent.size,
  )
