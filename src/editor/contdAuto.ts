...
export function computeContdChanges(
  ...
  for (const child of blocks) {
    if (child.type === 'character') {
      ...
      const hasContd =
        /\(CONT'D\)|\(CONT'D\)|\(CONTD\)/i.test(raw) ||
        raw.includes(contdMarkerUpper)
      const contdAuto = child.attrs.contdAuto === true
      const contdSuppressed = child.attrs.contdSuppressed === true
      const shouldHaveContd =
        lastCharBase !== null && base === lastCharBase && !lastWasDialogue
      ...
      if (shouldHaveContd && base) {
        ...
        if (contdAuto || contdSuppressed)
          setAttrs({ contdAuto: false, contdSuppressed: false })
      }
