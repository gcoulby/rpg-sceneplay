...
import { getTextLines } from '../utils/open-draft/wrapText'
...
export function getPageMetrics(layout: PageLayout) {
  const contentHeightPt =
    layout.pageHeight * 72 - layout.topMargin - layout.bottomMargin
  ...
  return new Plugin({
    ...
    props: {
      decorations(state) {
        ...
        const showDialogueBreakContd =
          resolveMoresContds(layout).dialogueBreakContd
        ...
        for (const brk of ps.breaks) {
          ...
          const contdPx =
            brk.isDialogueSplit && showDialogueBreakContd ? lineHeightPx : 0
...
function computeBreaks(
  ...
  while (i < nodes.length) {
    ...
    const isTitleRegionNode =
      node.typeName === 'titlePage' || node.typeName === 'screenplayImage'
    ...
    const forceBreak =
      lineCount > 0 && hints.forceBreakBefore.has(node.elementId)

    if (
      (forceBreak || lineCount + blockLines > linesPerPage) &&
      lineCount > 0
    ) {
      ...
      if (node.typeName === 'character' && blockEnd > i) {
        const charLines =
          node.spaceBefore +
          getTextLines(node.text, CHARS_PER_LINE[node.typeName] || 41)
        ...
        if (remaining >= charLines + MIN_DL) {
          ...
          if (lastFittedNode > i && fittedDL >= MIN_DL && remainDL >= MIN_DL) {
            ...
            const splitNode =
              splitIdx < nodes.length ? nodes[splitIdx] : nodes[blockEnd]
            breaks.push({
              nodeIndex: Math.min(splitIdx, nodes.length - 1),
              offset: splitNode.offset,
              nodeSize: splitNode.nodeSize,
              pageNumber,
              linesOnPage: lineCount,
            ...
            for (let j = splitIdx; j <= blockEnd; j++) {
              ...
              lineCount +=
                j === splitIdx
                  ? getTextLines(dn.text, dc)
                  : dn.spaceBefore + getTextLines(dn.text, dc)
            }
            i = blockEnd + 1
            continue
          }
...
export function computePageBlocks(
  ...
  if (breaks.length === 0) {
    ...
    for (let i = 0; i < breaks.length; i++) {
      const endNode =
        i + 1 < breaks.length ? breaks[i + 1].nodeIndex - 1 : nodes.length - 1
      pageBounds.push({
  ...
  for (const pb of pageBounds) {
    ...
    for (
      let i = pb.startNode;
      i <= Math.min(pb.endNode, nodes.length - 1);
      i++
    ) {
      const node = nodes[i]
      const cpl = CHARS_PER_LINE[node.typeName] || 62
