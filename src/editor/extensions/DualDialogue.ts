    ...
    return {
      toggleDualDialogue:
        ...
        ({ editor, tr, dispatch }) => {
          ...
          while (
            g1Start > 0 &&
            DIALOGUE_NODE_TYPES.has(blocks[g1Start - 1].node.type.name)
          ) {
            g1Start--
          }
          if (blocks[g1Start].node.type.name !== 'character') {
            for (let i = curIdx; i >= 0; i--) {
              if (blocks[i].node.type.name === 'character') {
                g1Start = i
                break
              }
          ...
          let g2End = -1
          if (
            g2Start < blocks.length &&
            blocks[g2Start].node.type.name === 'character'
          ) {
            g2End = g2Start
          ...
          if (g2End < 0) {
            ...
            if (
              ...
              while (
                prevStart > 0 &&
                DIALOGUE_NODE_TYPES.has(blocks[prevStart - 1].node.type.name)
              ) {
                prevStart--
              }
