...
import { jsonBlockText } from '../utils/open-draft/nodeText'
...
function renderClusterIndependently(
  ...
  for (const idx of deletedIndices) {
    ...
    if (base.text) {
      ...
      decorations.push(
        Decoration.widget(deletionPos, () => createDeletedBlock(text, type), {
          side: -1,
        }),
      )
    }
...
function diffCluster(
  ...
  const isMultiBlock = deletedIndices.length > 1 || insertedIndices.length > 1
...
function computeDecorations(
  ...
  while (i < nodeOps.length) {
    ...
    if (nextOp && nextOp.type === 'equal') {
      ...
      const lastInsert = currBlocks[insertedIndices[insertedIndices.length - 1]]
