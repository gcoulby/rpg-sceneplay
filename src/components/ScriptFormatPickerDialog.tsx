...
const ScriptFormatPickerDialog: React.FC<Props> = ({ enabledIds, onPick, onCancel }) => {
  ...
  return (
    <div className="dialog-overlay fixed left-0 top-0 right-0 bg-black/50 z-3000 flex items-start justify-center overflow-y-auto h-(--vv-height,100dvh) pt-[5vh] px-4 pb-4" onClick={onCancel}>
      <div className="bg-(--fd-dropdown-bg) border border-(--fd-border) rounded-lg shadow-[0_8px_32px_rgba(0,0,0,0.6)] w-115 max-w-[92vw] max-h-[85vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="py-3.5 px-5 border-b border-(--fd-border) font-semibold text-base shrink-0 text-(--fd-text)">Choose script format</div>
        <div className="py-3.5 px-5 overflow-y-auto flex-1">
          {options.length === 0 ? (
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold text-(--fd-text) flex items-center gap-2">
                      <span>{tpl.name}</span>
                      {tpl.scriptTypeGroup && (
                        <span className="text-[10px] font-semibold uppercase tracking-[0.5px] py-px px-1.5 rounded-[3px] bg-white/6 text-(--fd-text-muted)">{tpl.scriptTypeGroup}</span>
                      )}
                    </div>
