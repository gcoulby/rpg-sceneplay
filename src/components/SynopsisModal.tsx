  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="py-4 px-5 flex-1 flex flex-col gap-2.5 max-[600px]:px-4 max-[600px]:py-3">
          <div className="text-[13px] font-semibold text-(--fd-text) opacity-70 overflow-hidden text-ellipsis whitespace-nowrap max-[600px]:whitespace-normal">{sceneHeading}</div>
          {(pageLength != null || autoTimingSeconds != null) && (
            <div className="flex gap-2.5 text-xs text-(--fd-text-muted) tabular-nums mb-0.5">
              ...
              {finalSeconds > 0 && (
                <span className="font-semibold" style={{ color: getTimingColor(finalSeconds) }}>
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-xs text-(--fd-text-muted) whitespace-nowrap">Scene Duration</span>
            <div className="flex items-center gap-4 flex-wrap">
              <label className="flex items-center gap-1 text-[13px] text-(--fd-text) cursor-pointer">
                <input type="radio" name="timing" className="cursor-pointer" checked={timingMode === 'auto'} onChange={() => setTimingMode('auto')} />
                Auto{autoTimingSeconds ? ` (${formatSceneDuration(autoTimingSeconds)})` : ''}
              </label>
              <label className="flex items-center gap-1 text-[13px] text-(--fd-text) cursor-pointer">
                <input type="radio" name="timing" className="cursor-pointer" checked={timingMode === 'manual'} onChange={() => {
              ...
              {timingMode === 'manual' && (
                <div className="flex items-center gap-1">
                  <input
                    ...
                    max="99"
                  />
                  <span className="text-xs text-(--fd-text-muted) font-medium">m</span>
                  <input
                    type="number"
                    className="w-12 h-7 bg-(--fd-input-bg) text-(--fd-text) border border-(--fd-border) rounded-[3px] px-1.5 text-[13px] text-center outline-none tabular-nums focus:border-(--fd-accent)"
                    value={manualSeconds}
                    ...
                    max="59"
                  />
                  <span className="text-xs text-(--fd-text-muted) font-medium">s</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="flex flex-col gap-1.5 py-2 px-5">
          <span className="text-xs text-(--fd-text-muted) whitespace-nowrap">Scene Color</span>
        ...
        <div className="dialog-footer flex items-center gap-2 px-5 py-3.5 border-t border-(--fd-border) shrink-0">
          ...
          <button className="dialog-btn dialog-btn-primary h-8.5 px-4.5 bg-(--fd-accent) border border-(--fd-accent) text-white rounded text-sm cursor-pointer hover:opacity-90 disabled:opacity-50 disabled:cursor-default" onClick={handleSave}>Save</button>
        </div>
      </div>
    </div>
  );
