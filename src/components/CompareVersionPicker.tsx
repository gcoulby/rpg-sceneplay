...
const CompareVersionPicker: React.FC<CompareVersionPickerProps> = ({
  ...
  return (
    <div className="dialog-overlay fixed inset-x-0 top-0 z-3000 flex items-start justify-center h-[var(--vv-height,100dvh)] pt-[5vh] px-4 pb-4 overflow-y-auto bg-black/50 max-[480px]:pt-[env(safe-area-inset-top,0px)] max-[480px]:px-0 max-[480px]:pb-0" onClick={onClose}>
      ...
      >
        <div className="py-3.5 px-5 border-b border-(--fd-border) font-semibold text-base shrink-0 text-(--fd-text)">Compare with Version</div>
        <div className="p-5 overflow-y-auto flex-auto">
          {!currentProject && (
            <p className="py-4 text-(--fd-text-muted) text-xs italic text-center">No project selected.</p>
          )}
          {error && <p className="py-4 text-[#ff6b6b] text-xs italic text-center">{error}</p>}
          {loading && <p className="py-4 text-(--fd-text-muted) text-xs italic text-center">Loading versions...</p>}
