  ...
  return (
    <div className="flex flex-col h-full w-full bg-(--fd-background) text-(--fd-text)">
      <div className="flex items-center gap-3.5 py-2.5 px-4.5 border-b border-(--fd-border) shrink-0 flex-wrap">
        <div className="flex items-center gap-2 flex-1 min-w-50 text-xs">
          <span className="flex items-center gap-1">
            <span className="font-bold py-px px-1.75 rounded-[3px] text-[10px] text-white bg-[#64748b]">A</span>
            {labelA}
          </span>
          <span className="text-(--fd-text-muted)">→</span>
          <span className="flex items-center gap-1">
            <span className="font-bold py-px px-1.75 rounded-[3px] text-[10px] text-white bg-[#3b82f6]">B</span>
            {labelB}
          </span>
        </div>
        <div className="flex items-center gap-1">
          ...
          {onClose && (
            <button className="bg-none border-none text-(--fd-text-muted) text-xl cursor-pointer py-0 px-2 leading-none hover:text-(--fd-text)" onClick={onClose} title="Close diff">×</button>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="flex-1 overflow-auto py-4.5 px-7 font-['Courier_New',Courier,monospace] text-[12pt] bg-(--fd-page-bg,white) text-[#222]">
          {displayBlocks.length === 0 ? (
            <div className="p-10 text-center italic text-[#888]">No differences.</div>
          ) : mode === 'side-by-side' ? (
        ...
        {showSummary && (
          <div className="w-70 shrink-0 border-l border-(--fd-border) bg-(--fd-navigator-bg) p-4 overflow-y-auto text-(--fd-text) font-[inherit] text-xs">
            <h4 className="m-0 mb-2.5 text-[13px] uppercase tracking-[0.04em] text-(--fd-text)">Summary</h4>
            <div className="flex items-center gap-2 py-1 text-(--fd-text-muted)">
              <span className="bg-[#10b981] text-white py-px px-2 rounded-[3px] font-bold text-[11px] min-w-8 text-center">+{diff.summary.totalAdded}</span>
              added
            </div>
            <div className="flex items-center gap-2 py-1 text-(--fd-text-muted)">
              <span className="bg-[#ef4444] text-white py-px px-2 rounded-[3px] font-bold text-[11px] min-w-8 text-center">−{diff.summary.totalDeleted}</span>
              deleted
            </div>
            <div className="flex items-center gap-2 py-1 text-(--fd-text-muted)">
              <span className="bg-[#f59e0b] text-white py-px px-2 rounded-[3px] font-bold text-[11px] min-w-8 text-center">~{diff.summary.totalModified}</span>
              modified
            </div>
            {diff.summary.scenesChanged.length > 0 && (
              <>
                <h5 className="mt-4 mb-1.5 text-[11px] uppercase tracking-[0.04em] text-(--fd-text-muted)">Scenes changed ({diff.summary.scenesChanged.length})</h5>
                <ul className="list-none p-0 m-0">
                  {diff.summary.scenesChanged.slice(0, 20).map((s, i) => (
                    <li key={i} className="py-0.75 text-[11px] flex items-center gap-1.5">{s}</li>
                  ))}
                  {diff.summary.scenesChanged.length > 20 && (
                    <li className="py-0.75 text-[11px] flex items-center gap-1.5"><em>+{diff.summary.scenesChanged.length - 20} more</em></li>
                  )}
                </ul>
              </>
            )}
            {diff.summary.dialogueDelta.length > 0 && (
              <>
                <h5 className="mt-4 mb-1.5 text-[11px] uppercase tracking-[0.04em] text-(--fd-text-muted)">Dialogue changes</h5>
                <ul className="list-none p-0 m-0">
                  {diff.summary.dialogueDelta.map((d) => (
                    <li key={d.character} className="py-0.75 text-[11px] flex items-center gap-1.5">
                      <strong>{d.character}</strong>
                      <span className="bg-[#10b981] text-white py-px px-2 rounded-[3px] font-bold text-[11px] min-w-8 text-center">+{d.added}</span>
                      <span className="bg-[#ef4444] text-white py-px px-2 rounded-[3px] font-bold text-[11px] min-w-8 text-center">−{d.removed}</span>
                    </li>
                  ))}
                </ul>
              </>
...
const DIFF_BLOCK_BASE = 'py-2 px-3 border-l-[3px] border-transparent rounded-sm relative min-h-[30px]';
...
const BlockView: React.FC<{ side: 'a' | 'b'; block: DiffBlock }> = ({ side, block }) => {
  ...
  return (
    <div className={`${DIFF_BLOCK_BASE} ${DIFF_TYPE_CLASSES[block.type]}`}>
      <div className="text-[9px] uppercase text-[#888] tracking-wider mb-0.75 font-sans">{typeLabel}</div>
      <div className={`font-['Courier_New',Courier,monospace] whitespace-pre-wrap wrap-break-word ${DIFF_EL_CONTENT_CLASSES[block.elementType] || ''}`}>
  ...
  <>
    {diffs.map((d, i) => {
      ...
      if (d.kind === 'removed' && side === 'a') {
        return <span key={i} className="bg-[rgba(239,68,68,0.25)] line-through text-[#888]">{d.text}</span>;
      }
      if (d.kind === 'added' && side === 'b') {
        return <span key={i} className="bg-[rgba(16,185,129,0.3)] border-b-2 border-[#10b981] no-underline">{d.text}</span>;
      }
      return null;
    })}
  </>
...
const UnifiedDiff: React.FC<{ blocks: DiffBlock[] }> = ({ blocks }) => (
  <div className="flex flex-col gap-1">
    {blocks.map((b, i) => {
      ...
      if (b.type === 'unchanged') {
        return (
          <div key={i} className={`${DIFF_BLOCK_BASE} ${DIFF_TYPE_CLASSES.unchanged}`}>
            <div className="text-[9px] uppercase text-[#888] tracking-wider mb-0.75 font-sans">{typeLabel}</div>
            <div className={`font-['Courier_New',Courier,monospace] whitespace-pre-wrap wrap-break-word ${elClass}`}>{b.oldText}</div>
          </div>
        );
      }
      if (b.type === 'modified' && b.wordDiffs) {
        return (
          <div key={i} className={`${DIFF_BLOCK_BASE} ${DIFF_TYPE_CLASSES.modified}`}>
            <div className="text-[9px] uppercase text-[#888] tracking-wider mb-0.75 font-sans">{typeLabel} (modified)</div>
            <div className={`font-['Courier_New',Courier,monospace] whitespace-pre-wrap wrap-break-word ${elClass}`}>
      ...
      if (b.type === 'deleted') {
        return (
          <div key={i} className={`${DIFF_BLOCK_BASE} ${DIFF_TYPE_CLASSES.deleted}`}>
            <div className="text-[9px] uppercase text-[#888] tracking-wider mb-0.75 font-sans">{typeLabel} (deleted)</div>
            <div className={`font-['Courier_New',Courier,monospace] whitespace-pre-wrap wrap-break-word ${elClass}`}><del>{b.oldText}</del></div>
          </div>
        );
      }
      return (
        <div key={i} className={`${DIFF_BLOCK_BASE} ${DIFF_TYPE_CLASSES.added}`}>
          <div className="text-[9px] uppercase text-[#888] tracking-wider mb-0.75 font-sans">{typeLabel} (added)</div>
          <div className={`font-['Courier_New',Courier,monospace] whitespace-pre-wrap wrap-break-word ${elClass}`}>{b.newText}</div>
        </div>
      );
    })}
  </div>
