...
const RelForm: React.FC<RelFormProps> = ({
  characterName,
  allCharacters,
  selectBoth,
  existing,
  ...
  const fieldClass =
    'w-full py-[5px] px-2 border border-(--fd-border) rounded text-(--fd-text) bg-(--fd-input-bg) font-[inherit] text-xs'
  const labelClass =
    'block text-[10px] font-semibold uppercase tracking-[0.3px] text-(--fd-text-muted) mb-[3px]'
  ...
  return (
    <div className="bg-(--fd-dropdown-bg) border border-(--fd-border) rounded-lg p-4 min-w-70 max-w-90 shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
      ...
      <div className="mb-2">
        ...
        <textarea
          className={`${fieldClass} resize-y min-h-10`}
          value={desc}
      ...
      <div className="flex justify-end gap-1.5 mt-3">
        ...
        <button
          className="py-1 px-2.5 rounded text-[11px] cursor-pointer whitespace-nowrap bg-(--fd-accent) border border-(--fd-accent) text-white hover:opacity-90 disabled:opacity-40 disabled:cursor-default"
  ...
  return (
    ...
    >
      ...
      {(addingFrom || editingRel) && (
        <div
          className="z-10 absolute inset-0 flex justify-center items-center bg-black/40"
          onClick={() => {
            setAddingFrom(null)
            setEditingRel(null)
          }}
        >
          <div onClick={(e) => e.stopPropagation()}>
            ...
            {editingRel && (
              <button
                className="bg-transparent hover:bg-[rgba(239,83,80,0.1)] px-2.5 py-1 border border-[#ef5350] rounded text-[#ef5350] text-[11px] whitespace-nowrap cursor-pointer"
