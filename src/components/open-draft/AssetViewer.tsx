...
const AssetViewer: React.FC<AssetViewerProps> = ({
  ...
  const renderPreview = () => {
    ...
    if (mime.startsWith('video/')) {
      return (
        <video controls className="rounded max-w-full max-h-[60vh]">
          <source src={assetUrl} type={mime} />
    ...
    return (
      <div className="p-6 text-center">
        ...
        <div className="text-sm font-medium text-(--fd-text) mb-1">
          {asset.original_name}
        </div>
        ...
        <a
          ...
          className="inline-block py-2 px-4 bg-(--fd-accent) text-white no-underline rounded text-[13px] font-medium hover:opacity-85"
        >
          Download File
        </a>
      </div>
  ...
  return (
    <div
      className="z-3500 fixed inset-0 flex justify-center items-center bg-black/70 asset-viewer-overlay"
      onClick={onClose}
    >
      <div
        className="bg-(--fd-dropdown-bg) border border-(--fd-border) rounded-lg shadow-[0_8px_32px_rgba(0,0,0,.6)] w-175 max-w-[90vw] max-h-[80vh] flex flex-col overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between items-center py-2.5 px-4 border-b border-(--fd-border) shrink-0">
          <span className="text-[13px] font-semibold text-(--fd-text) overflow-hidden text-ellipsis whitespace-nowrap">
            {asset.original_name}
          </span>
          <button
...
const TextPreview: React.FC<{ url: string }> = ({ url }) => {
  ...
  return (
    <pre className="w-full max-h-[60vh] overflow-auto bg-[#1a1a1a] text-(--fd-text) p-4 rounded font-[Courier_New,Courier,monospace] text-xs leading-normal whitespace-pre-wrap wrap-break-word">
      {text}
    </pre>
  )
}
