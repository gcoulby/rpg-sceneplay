...
const CollabLoginDialog: React.FC<CollabLoginDialogProps> = ({
  onClose,
  onSuccess,
}) => {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      showToast(
        r.message || 'If an account exists, a reset link was sent.',
        'success',
      )
      setForgotMode('sent')
    } catch (err: any) {
    ...
    >
      ...
      >
        ...
        <div className="flex-1 p-5 overflow-y-auto dialog-body">
          ...
          >
            A collaboration account is required to use real-time editing. You
            can manage your account in System Settings.
          </p>

          {isDemoServer && (
            <div className="bg-[#fef3cd] [&_code]:bg-black/8 mb-4 px-4 [&_code]:px-1.5 py-3 [&_code]:py-px border border-[#ffc107] rounded-lg [&_code]:rounded-[3px] [&_strong]:font-semibold text-[#856404] text-[13px] [&_code]:text-xs leading-normal">
          ...
          {tab === 'login' ? (
            forgotMode === 'form' ? (
              <div className="flex flex-col gap-3.5 settings-auth-form">
                <p
                  style={{
                    margin: '0 0 12px',
                    fontSize: 13,
                ...
                >
                  Enter the email address you used to sign up. We'll email you a
                  link to choose a new password. The link expires in 30 minutes.
                </p>
                <div className="flex flex-col gap-1.5 settings-field">
                  <label>Email</label>
                  <input
              ...
              <div className="flex flex-col gap-3.5 settings-auth-form">
                <p style={{ margin: '0 0 12px', fontSize: 13 }}>
                  If an account with <strong>{forgotEmail}</strong> exists,
                  we've emailed it a link to choose a new password. Check your
                ...
                >
                  The link expires in 30 minutes. You can request a new one at
                  any time.
                </p>
                <button
                  className="dialog-btn dialog-btn-primary settings-auth-submit mt-1 self-start min-w-40 h-9 text-sm bg-(--fd-accent) text-white border-none rounded-md font-semibold cursor-pointer hover:opacity-85 disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              <div className="flex flex-col gap-3.5 settings-auth-form">
                ...
                >
                  ...
                  <strong>{pendingChallenge.email}</strong> to confirm this new
              ...
              <div className="flex flex-col gap-3.5 settings-auth-form">
                ...
                <div className="flex flex-col gap-1.5 settings-field">
                  ...
                  <div className="relative flex items-center password-input-wrapper">
                    <input
                      className="dialog-input h-9 bg-(--fd-input-bg) text-(--fd-text) border border-(--fd-border) rounded px-2.5 text-sm outline-none w-full box-border focus:border-(--fd-accent)"
          ...
          <div className="mt-3 collab-remember-section">
          ...
          {serverConfig?.googleEnabled && (
            <div className="mt-5 settings-google-section">
              <div className="settings-divider flex items-center gap-3 mb-5 text-(--fd-text-muted) text-[13px] before:content-[''] before:flex-1 before:h-px before:bg-(--fd-border) after:content-[''] after:flex-1 after:h-px after:bg-(--fd-border)">
                <span>or</span>
              </div>
              <button
