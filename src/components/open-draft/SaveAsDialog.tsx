...
const ImportedSourceNotice: React.FC = () => {
  ...
  return (
    <div className="py-2.5 px-3 mb-3 border border-[rgba(46,125,215,0.4)] bg-[rgba(46,125,215,0.10)] rounded-md text-xs text-(--fd-text) leading-[1.45]">
      <strong>Note:</strong> This document was imported from{' '}
      <strong>{importedSource.name}</strong> ({importedSource.format}). Saving
      creates a new file inside OpenDraft's library — it does{' '}
      <strong>not</strong> overwrite the original source file. To write back to
      ...
      try {
        ...
        setProjectName((current) => {
          ...
          if (
            defaultProjectName &&
            list.some(
              (p) => p.name.toLowerCase() === defaultProjectName.toLowerCase(),
            )
          ) {
          ...
          if (
            lastUsed &&
            list.some((p) => p.name.toLowerCase() === lastUsed.toLowerCase())
          ) {
            return lastUsed
          }
          if (list.length > 0) {
        ...
        if (!cancelled) {
          setProjectName(
            (current) => current || defaultProjectName || 'My Project',
          )
        }
      }
    })()
    ...
    <div className="dialog-overlay" onClick={onClose}>
      ...
      >
        ...
        <div className="dialog-body">
          ...
          {!WEB_ONLY_CLOUD && (
            <div className="mb-3 dialog-row">
              ...
              {destination === 'cloud' && !signedIn && (
                <div className="mt-1.5 text-[#ff9966] text-xs">
                  Sign in to save to OpenDraft Cloud — pressing Save will open
                  the login dialog.
                </div>
              )}
            </div>
            ...
            <div ref={comboRef} className="relative">
              <div className="flex gap-0">
                <input
                  ...
                  className="flex-1 rounded-r-none rounded-l-sm"
                />
                <button
                  type="button"
                  onClick={() => {
              ...
              {dropdownOpen && filteredProjects.length > 0 && (
                <div className="absolute top-full left-0 right-0 max-h-40 overflow-y-auto bg-(--fd-menu-bg,var(--fd-bg)) border border-(--fd-border) rounded mt-0.5 z-10 shadow-[0_4px_12px_rgba(0,0,0,.3)]">
          ...
          {error && <div className="mt-2 text-[#ff6b6b] text-xs">{error}</div>}
        </div>
