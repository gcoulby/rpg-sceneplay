...
const ShareDialog: React.FC<ShareDialogProps> = ({
  ...
  useEffect(() => {
    ...
    if (isCollabActive) {
      api
        .listCollabSessions(projectId, scriptId)
        .then(setSessions)
        .catch(() => {})
    }
  ...
  const handleGenerate = async () => {
    ...
    if (!isCollabAuthenticated()) {
      showToast(
        'Session expired — please log in again (Settings → Collaboration)',
        'error',
      )
      onClose()
      return
    ...
    try {
      ...
      if (!serverOk) {
        showToast(
          'Cannot reach the collaboration server. Make sure it is running.',
          'error',
        )
        setGenerating(false)
        return
      ...
      const existingNonce =
        sessions.length > 0 ? sessions[0].session_nonce || '' : ''
      const session = await api.createCollabInvite(
        projectId,
        scriptId,
        trimmed,
        role,
  ...
  return (
    <div
      ...
      onClick={(e) => {
        if (e.target === e.currentTarget && overlayPointerDown.current)
          onClose()
    ...
    >
      ...
      >
        ...
        <div className="flex-1 p-5 overflow-y-auto">
          {isCollabActive && (
            <div className="flex items-center gap-2 bg-[#1a3a2a] mb-3.5 px-3.5 py-2.5 rounded-md font-medium text-[#4ade80] text-sm collab-status-badge">
              <span className="collab-dot" /> Live collaboration active
            </div>
          )}

          <p className="mb-3 text-sm text-(--fd-text-muted)">
            Generate a link for each collaborator. They can open the link to
            join a live editing session.
          </p>

          <label className="block text-sm text-(--fd-text-muted) mb-1">
          ...
          <div className="flex gap-6 mt-1">
            <div className="flex-1">
              <label className="block text-sm text-(--fd-text-muted) mb-1 mt-3">
                Permission
              </label>
          ...
          {sessions.length > 0 && (
            <div className="max-h-[280px] overflow-y-auto">
              ...
              {sessions.map((s) => (
                <div
                  key={s.token}
                  className="flex items-center justify-between px-3 py-2.5 mt-2 rounded-md bg-(--fd-input-bg) gap-2.5"
                >
                  <div className="flex flex-col gap-[3px] min-w-0">
                    <span className="font-semibold text-sm text-(--fd-text)">
                      {s.collaborator_name}
                      <span
                        className={`collab-role-${s.role || 'editor'} inline-block text-[10px] px-1.5 py-px rounded-[3px] ml-2 font-medium uppercase tracking-[0.3px] ${s.role === 'viewer' ? 'bg-[rgba(255,183,77,0.15)] text-[#ffb74d]' : 'bg-[rgba(74,158,255,0.15)] text-[#4a9eff]'}`}
                      >
                  ...
                  <div className="flex gap-2 shrink-0">
                    ...
                    <button
                      className="bg-transparent hover:bg-[#3a2020] px-3 py-1.5 border border-[#5a3030] rounded text-[#e06060] text-[13px] whitespace-nowrap cursor-pointer collab-revoke-btn"
        ...
        <div className="dialog-footer flex items-center gap-2 px-5 py-3.5 border-t border-(--fd-border) shrink-0">
          ...
          <div className="flex-1" />
          <button
            className="dialog-btn h-[34px] px-[18px] bg-(--fd-toolbar-bg) text-(--fd-text) border border-(--fd-border) rounded cursor-pointer text-sm hover:bg-(--fd-toolbar-hover)"
            onClick={onClose}
          >
            Close
          </button>
