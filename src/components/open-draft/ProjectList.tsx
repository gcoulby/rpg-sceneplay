...
const SortableCard: React.FC<SortableCardProps> = ({
  ...
  return (
    ...
    >
      ...
      {project.color && (
        <div
          className="top-0 absolute inset-x-0 rounded-t-lg h-1"
      ...
      {sortKey === 'custom' && (
        <div
          className="cursor-grab text-(--fd-text-muted) text-sm py-0.5 px-1.5 rounded-[3px] select-none shrink-0 hover:text-(--fd-text) hover:bg-(--fd-overlay-subtle) active:cursor-grabbing"
          {...attributes}
          {...listeners}
      ...
      <div
        ...
        onClick={() => !editing && onNavigate(project.id)}
      >
        {editing ? (
          <input
        ...
        <div className="text-[13px] text-(--fd-text-muted) flex items-center gap-1.5 leading-normal">
          <span>
            {project.script_count} screenplay
            {project.script_count !== 1 ? 's' : ''}
          </span>
      ...
      {/* Action buttons */}
      <div className="flex gap-1 max-md:gap-1.5 mt-2.5">
        <button
        ...
        >
          <span
            className="inline-block border border-[rgba(255,255,255,0.15)] rounded-full w-3 h-3"
      ...
      {showActions && (
        <div
          className="script-actions-dropdown script-card-dropdown absolute right-2 top-auto bottom-14 mt-0 bg-(--fd-toolbar-bg) border border-(--fd-border) rounded-md py-1 z-200 min-w-45 max-md:min-w-50 shadow-[0_4px_12px_rgba(0,0,0,0.3)]"
          onClick={(e) => e.stopPropagation()}
          role="menu"
        >
          <div
          ...
          <div
            className="hover:bg-[rgba(255,107,107,0.1)] px-3.5 max-md:px-3.5 py-2 max-md:py-3 max-md:min-h-11 text-[#ff6b6b] text-[13px] max-md:text-sm transition-colors duration-100 cursor-pointer"
            role="menuitem"
            onClick={() => {
              setShowActions(false)
              onDelete(project.id)
      ...
      {showColorPicker && (
        <div
          className="absolute right-2.5 bottom-2.5 flex flex-wrap gap-1 bg-(--fd-toolbar-bg) border border-(--fd-border) rounded-md p-1.5 z-100 w-35 shadow-[0_4px_12px_rgba(0,0,0,0.3)]"
          onClick={(e) => e.stopPropagation()}
        >
          {ITEM_COLORS.map((c) => (
            <button
...
const ProjectList: React.FC = () => {
  ...
  const [source, setSource] = useState<ProjectSource>(() => {
    ...
    return (
      (localStorage.getItem('opendraft:projectSource') as ProjectSource) ||
      'local'
    )
  })
  const [sortKey, setSortKey] = useState<SortKey>(() => {
    return (
      (localStorage.getItem('opendraft:projectSort') as SortKey) || 'custom'
    )
  })
  ...
  useEffect(() => {
    if (!WEB_ONLY_CLOUD) {
      try {
        localStorage.setItem('opendraft:projectSource', source)
      } catch {
        /* ignore */
      }
    }
  ...
  return (
    <div className="h-full bg-(--fd-bg) text-(--fd-text) p-10 overflow-y-auto box-border max-md:px-3 max-md:py-4">
      {!WEB_ONLY_CLOUD && (
        <div
          className="flex gap-0 max-w-300 mx-auto mb-6 border-b border-(--fd-border) max-md:mb-4 max-md:overflow-x-auto max-md:scrollbar-none max-md:[&::-webkit-scrollbar]:hidden"
          role="tablist"
        >
          <button
            type="button"
      ...
      <div className="flex max-md:flex-col justify-between items-center max-md:items-stretch max-md:gap-3 mx-auto mb-10 max-md:mb-5 max-w-300">
        <div className="flex flex-col gap-1">
          <h1 className="text-[28px] font-bold text-(--fd-text) tracking-[-0.5px] max-md:text-[22px] max-[480px]:text-[20px]">
            Projects
          </h1>
          <span className="text-sm text-(--fd-text-muted)">
            ...
            {source === 'cloud' && getApiBase() && (
              <>
                {' · '}
                <span title="Server this app is talking to">
                  {getApiBase()}
                </span>
              </>
            )}
        ...
        <div className="flex max-md:flex-wrap items-center gap-2.5 max-md:gap-2">
          <select
      ...
      {loading ? (
        ...
        <div className="max-w-300 my-20 mx-auto text-center text-(--fd-text-muted) text-sm">
          <div className="opacity-50 mb-4 text-[48px]">&#9729;</div>
          <div>
            Sign in to access OpenDraft Cloud projects. Click the "Local only"
            indicator at the top to sign in.
          </div>
        </div>
      ) : errorMsg ? (
        <div className="max-w-300 my-20 mx-auto text-center text-(--fd-text-muted) text-sm">
        ...
        >
          {pinnedProjects.length > 0 && (
            <>
              <div className="mx-auto mt-4 first:mt-0 mb-1 py-2 max-w-300 font-semibold text-[#f4d35e] text-[11px] uppercase tracking-[0.8px]">
                Pinned
              </div>
              <SortableContext
              ...
              >
                <div className="gap-4 max-md:gap-3 grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] max-md:grid-cols-1 mx-auto max-w-300">
          ...
          {unpinnedProjects.length > 0 && (
            <>
              <div className="text-[11px] font-semibold uppercase tracking-[0.8px] py-2 mt-4 mb-1 max-w-300 mx-auto first:mt-0 text-(--fd-text-muted)">
                All Projects
              </div>
              <SortableContext
      ...
      {showNewDialog && (
        <div
          className="fixed left-0 top-0 right-0 bg-black/50 z-3000 flex items-start justify-center h-(--vv-height,100dvh) pt-[5vh] px-4 pb-4 overflow-y-auto"
          onClick={() => setShowNewDialog(false)}
        >
          <div
            className="dialog-box bg-(--fd-dropdown-bg) border border-(--fd-border) rounded-lg shadow-[0_8px_32px_rgba(0,0,0,0.6)] min-w-[320px] max-w-[400px] max-h-[calc(var(--vv-height,100dvh)-48px)] flex flex-col max-md:min-w-0 max-md:max-w-none max-md:w-[calc(100vw-32px-env(safe-area-inset-left,0px)-env(safe-area-inset-right,0px))]"
            onClick={(e) => e.stopPropagation()}
          >
        ...
        >
          ...
          >
            ...
            <div className="dialog-actions flex justify-end gap-2 py-3.5 px-5 border-t border-(--fd-border) shrink-0 [&_button]:h-8.5 [&_button]:px-4.5 [&_button]:bg-(--fd-toolbar-bg) [&_button]:text-(--fd-text) [&_button]:border [&_button]:border-(--fd-border) [&_button]:rounded [&_button]:cursor-pointer [&_button]:text-sm [&_button:hover]:bg-(--fd-menu-hover)">
              ...
              <button
                className="bg-[#c0392b]! hover:opacity-90 border-[#c0392b]! text-white!"
