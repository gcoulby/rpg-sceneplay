  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (useSettingsStore.getState().collabAuth.accessToken) {
        ...
        useSettingsStore
          .getState()
          .setCollabAuth({ ...current, user: refreshed })
      } else {
        if (!user?.email) {
          showToast('No account in progress. Please sign up again.', 'error')
          return
        }
        const resp = await collabAuthApi.verifyEmailLink(
          user.email,
          code.trim(),
        )
      ...
      >
        ...
        <div className="flex-1 p-5 overflow-y-auto">
          <p className="mb-3 text-sm text-(--fd-text-muted)">
            ...
            {user?.email ? <strong>{user.email}</strong> : 'your email'}. Enter
            it below, or click the activation link in the email.
          </p>
          <input
