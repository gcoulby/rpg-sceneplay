 ...
 */
function extractTokenAndServer(input: string): {
  token: string
...
const JoinCollabDialog: React.FC<JoinCollabDialogProps> = ({
  onJoin,
  onClose,
}) => {
  ...
  const handleJoin = async () => {
    ...
    console.log('[JoinCollab] Extracted:', {
    ...
    try {
      ...
      try {
        ...
        console.error(
          '[JoinCollab] Step 1 error (collab server unreachable):',
          err,
        )
      }
      ...
      if (!session) {
        console.log(
          '[JoinCollab] Step 2: falling back to api.validateCollabSession',
        )
        try {
  ...
  return (
    <div className="dialog-overlay" onClick={onClose}>
      ...
      >
        <div className="dialog-header">Join Collaboration Session</div>

        <div className="dialog-body">
          <p className="mb-4 text-sm text-(--fd-text-muted)">
            Paste the collaboration link or token you received from the session
            host.
          </p>

          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-medium text-(--fd-text-muted)">
              Collaboration Link or Token
            </label>
            <input
              ref={inputRef}
