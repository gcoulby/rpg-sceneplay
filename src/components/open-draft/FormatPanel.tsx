...
const FormatPanel: React.FC<FormatPanelProps> = ({ editor, onClose }) => {
  ...
  const initialFont =
    (attrs.fontFamily as string) || rule?.fontFamily || pageFont
  const initialSizeStr = (attrs.fontSize as string) || ''
  const initialSize = initialSizeStr
    ? parseInt(initialSizeStr, 10) || (rule?.fontSize ?? pageFontSize)
    : (rule?.fontSize ?? pageFontSize)
    ...
    (opts: {
      ...
      if (DEFAULT_FONTS.includes(opts.f)) {
        ...
        editor
          ...
          .focus()
          .setTextSelection(sel)
          .setMark('textStyle', { fontFamily: opts.f })
          .run()
      }
  ...
  const preview = (
    ...
    applyPreview({
      ...
      s: overrides.s ?? size,
      b: overrides.b ?? bold,
      i: overrides.i ?? italic,
      u: overrides.u ?? underline,
      st: overrides.st ?? strike,
      sub: overrides.sub ?? subscript,
      sup: overrides.sup ?? superscript,
      col: overrides.col ?? textColor,
      hl: overrides.hl ?? highlightColor,
    })
  }
  ...
  const handleHighlightChange = (c: string) => {
    setHighlightColor(c)
    preview({ hl: c })
  }
  ...
  const nodeSel = editor.state.selection as unknown as {
    node?: { type: { name: string }; attrs: Record<string, unknown> }
  }
  const selImage =
    nodeSel.node && nodeSel.node.type.name === 'screenplayImage'
      ? nodeSel.node
      : null
  if (selImage) {
    ...
    return (
      <div
        className="z-3500 fixed inset-0 flex justify-center items-center bg-black/40 format-panel-overlay"
        onMouseDown={(e) => {
        ...
        >
          ...
          >
            ...
            <button
              ...
              onClick={onClose}
              aria-label="Close image properties"
            >
              &times;
            </button>
          </div>
          <div className="flex flex-col gap-3.5 p-4">
            <div className="flex items-center gap-3">
              <label className="text-xs text-(--fd-text-muted) w-10 shrink-0">
                Width (px)
              </label>
              <input
                type="number"
                ...
                onChange={(e) =>
                  setImgAttr({
                    width: Math.max(40, Number(e.target.value) || 40),
                  })
                }
              />
            </div>
                ...
                {(['left', 'center', 'right'] as const).map((al) => (
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: (a.align || 'center') === al ? 700 : 400,
                    }}
                  >
                    {al}
                  </button>
          ...
          <div className="flex justify-end gap-2 py-3 px-4 border-t border-(--fd-border)">
            <button
              className="h-[30px] px-5 rounded text-[13px] cursor-pointer border border-(--fd-accent) bg-(--fd-accent) text-white hover:opacity-90"
              onClick={onClose}
            >
              Done
            </button>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col gap-3.5 p-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-wrap gap-1">
              <button
                className={`format-style-btn w-8 h-[30px] max-[768px]:w-10 max-[768px]:h-[38px] border border-(--fd-border) rounded text-(--fd-text) text-sm cursor-pointer flex items-center justify-center hover:border-[#555] ${bold ? 'bg-(--fd-accent) border-(--fd-accent) text-white' : 'bg-(--fd-input-bg)'}`}
              ...
              <button
                className={`format-style-btn w-8 h-[30px] max-[768px]:w-10 max-[768px]:h-[38px] border border-(--fd-border) rounded text-(--fd-text) text-sm cursor-pointer flex items-center justify-center hover:border-[#555] ${italic ? 'bg-(--fd-accent) border-(--fd-accent) text-white' : 'bg-(--fd-input-bg)'}`}
              ...
              <button
                className={`format-style-btn w-8 h-[30px] max-[768px]:w-10 max-[768px]:h-[38px] border border-(--fd-border) rounded text-(--fd-text) text-sm cursor-pointer flex items-center justify-center hover:border-[#555] ${underline ? 'bg-(--fd-accent) border-(--fd-accent) text-white' : 'bg-(--fd-input-bg)'}`}
              ...
              <button
                className={`format-style-btn w-8 h-[30px] max-[768px]:w-10 max-[768px]:h-[38px] border border-(--fd-border) rounded text-(--fd-text) text-sm cursor-pointer flex items-center justify-center hover:border-[#555] ${strike ? 'bg-(--fd-accent) border-(--fd-accent) text-white' : 'bg-(--fd-input-bg)'}`}
              ...
              <button
                className={`format-style-btn w-8 h-[30px] max-[768px]:w-10 max-[768px]:h-[38px] border border-(--fd-border) rounded text-(--fd-text) text-sm cursor-pointer flex items-center justify-center hover:border-[#555] ${subscript ? 'bg-(--fd-accent) border-(--fd-accent) text-white' : 'bg-(--fd-input-bg)'}`}
              ...
              <button
                className={`format-style-btn w-8 h-[30px] max-[768px]:w-10 max-[768px]:h-[38px] border border-(--fd-border) rounded text-(--fd-text) text-sm cursor-pointer flex items-center justify-center hover:border-[#555] ${superscript ? 'bg-(--fd-accent) border-(--fd-accent) text-white' : 'bg-(--fd-input-bg)'}`}
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                title="Text color"
              />
              <span className="text-xs text-(--fd-text-muted) min-w-[60px]">
                {textColor || 'Default'}
              </span>
              {textColor && (
                <button
                  className="text-[11px] py-0.5 px-2 bg-(--fd-input-bg) border border-(--fd-border) rounded-[3px] text-(--fd-text) cursor-pointer hover:bg-(--fd-menu-hover)"
        ...
        <div className="flex justify-end gap-2 py-3 px-4 border-t border-(--fd-border)">
          <button
            className="h-[30px] px-5 rounded text-[13px] cursor-pointer border border-(--fd-border) bg-transparent text-(--fd-text-muted) hover:bg-white/5 hover:text-(--fd-text)"
            onClick={handleCancel}
          >
            Cancel
          </button>
