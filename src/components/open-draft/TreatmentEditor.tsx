...
const TreatmentEditor: React.FC = () => {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          'treatment-content min-h-[200px] outline-none focus:outline-none [&_.ProseMirror-focused]:outline-none',
      },
    },
  })
  ...
  const toolBtnBase =
    'bg-transparent border-none text-(--fd-text-muted) py-1.5 px-2 rounded-[3px] cursor-pointer flex items-center justify-center min-w-7 min-h-7 transition-all duration-100 hover:bg-(--fd-overlay-light) hover:text-(--fd-text)'
  const toolBtnActive =
    'bg-(--fd-accent) text-white border-none py-1.5 px-2 rounded-[3px] cursor-pointer flex items-center justify-center min-w-7 min-h-7'
      ...
      <div className="flex-1 overflow-y-auto py-10 bg-(--fd-canvas-bg,#1a1a1a)">
        <div className="treatment-page max-w-[720px] min-h-[900px] mx-auto bg-(--fd-page-bg,white) text-[#111] py-18 px-24 font-['Times_New_Roman',Times,serif] text-[12pt] leading-[2] shadow-[0_4px_20px_rgba(0,0,0,0.3)]">
          {editor ? (
            ...
            <div className="py-5 text-[#888] italic">Initializing editor…</div>
          )}
        </div>
      </div>
    </div>
