...
const ElementPicker: React.FC<ElementPickerProps> = ({
  ...
  const orderedTypes = useMemo<ElementType[]>(() => {
    ...
    return (ELEMENT_ORDER[defaultType] || DEFAULT_ORDER).filter((t) =>
      enabled.has(t),
    )
  ...
  return (
    ...
    >
      <div className="pt-1 px-3 pb-1.5 text-[10px] uppercase tracking-[0.5px] text-(--fd-text-muted) border-b border-(--fd-border) mb-0.5">
        Element Type
      </div>
