  ...
  useEffect(() => {
    ...
    const doScan = async () => {
      ...
      tr.setMeta(spellCheckPluginKey, {
        activeRange: { from: found[0].from, to: found[0].to },
      })
      tr.setSelection(
        TextSelection.near(editor.state.doc.resolve(found[0].from)),
      )
      tr.scrollIntoView()
      editor.view.dispatch(tr)
    }
        ...
        <div className="spell-modal-actions-col">
          ...
          {(() => {
            ...
            return targets.map((t) => {
              const label =
                t === PROJECT_DICT_TARGET ? 'Add to Project' : `Add to "${t}"`
              return (
