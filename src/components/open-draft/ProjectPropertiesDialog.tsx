...
const REGISTRATION_FIELDS: FieldDef[] = [
  ...
  {
    key: 'wga_registration_date',
    label: 'WGA Registration Date',
    type: 'text',
  },
  {
    key: 'copyright_registration',
...
const renderField = (
  ...
  >
    <label className="text-[11px] text-(--fd-text-muted) uppercase tracking-[0.4px]">
      {f.label}
    </label>
    {f.type === 'textarea' ? (
      <textarea
...
const ProjectPropertiesDialog: React.FC<Props> = ({
  project,
  onClose,
  onSaved,
}) => {
  const [name, setName] = useState(project.name)
  const [props, setProps] = useState<ProjectProperties>({
    ...EMPTY_PROPS,
    ...project.properties,
  })
  ...
  const addSubmission = () => {
    ...
    setProps((prev) => ({
      ...prev,
      submissions: [...(prev.submissions || []), entry],
    }))
  }
  ...
  return (
    ...
    >
      <div
        className="props-dialog bg-(--fd-dropdown-bg) border border-(--fd-border) rounded-lg shadow-[0_8px_32px_rgba(0,0,0,0.6)] w-145 max-h-[85vh] flex flex-col max-[768px]:w-[calc(100vw-32px-env(safe-area-inset-left,0px)-env(safe-area-inset-right,0px))] max-[768px]:max-w-145 max-[480px]:w-[calc(100vw-16px)]!"
      ...
      >
        ...
        <div className="flex-1 p-4 overflow-y-auto">
          ...
          <div className="gap-x-4 gap-y-2.5 grid grid-cols-2 max-[768px]:grid-cols-1">
            {FIELDS.map((f) =>
              renderField(f, props[f.key] as string, setField),
            )}
          </div>
          ...
          >
            <span
              className={`inline-block text-[9px] transition-transform duration-150 ${legalOpen ? 'rotate-90' : ''}`}
            >
              &#9654;
            </span>
            Registration &amp; Legal
          ...
          {legalOpen && (
            <>
              ...
              {/* Submission Log */}
              <div
                className="flex flex-col gap-0.75 col-span-full"
                style={{ marginTop: 12 }}
              >
                <label className="text-[11px] text-(--fd-text-muted) uppercase tracking-[0.4px]">
                  Submission Log
                ...
                {(props.submissions || []).length > 0 && (
                  <div className="flex flex-col gap-1 mb-2">
                    ...
                    {(props.submissions || []).map((sub) => (
                      <div
                        key={sub.id}
                        className="items-center gap-1.5 grid grid-cols-[110px_1fr_100px_100px_1fr_28px]"
                      >
                        <input
                          ...
                          value={sub.date}
                          onChange={(e) =>
                            updateSubmission(sub.id, 'date', e.target.value)
                          }
                        />
                        <input
                          className="h-6.5 bg-(--fd-input-bg,#222) text-(--fd-text) border border-(--fd-border) rounded-[3px] px-2 text-xs outline-none focus:border-(--fd-accent)"
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateSubmission(sub.id, 'type', e.target.value)
                          }
                        />
                        <input
                          className="h-6.5 bg-(--fd-input-bg,#222) text-(--fd-text) border border-(--fd-border) rounded-[3px] px-2 text-xs outline-none focus:border-(--fd-accent)"
