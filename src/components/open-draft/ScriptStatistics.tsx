...
const ScriptStatistics: React.FC<Props> = ({ editor }) => {
  ...
  return (
    <div className="flex flex-col flex-1 bg-[var(--fd-bg)] [&_.recharts-cartesian-axis-tick-value]:!fill-[var(--fd-text-muted)] [&_.recharts-label]:!fill-[var(--fd-text-muted)] [&_.recharts-pie-label-text]:!fill-[var(--fd-text-muted)] [&_.recharts-cartesian-axis-line]:!stroke-white/[0.08] [&_.recharts-cartesian-grid-horizontal_line]:!stroke-white/[0.08] [&_.recharts-cartesian-grid-vertical_line]:!stroke-white/[0.08] overflow-hidden [&_.recharts-legend-item-text]:!text-[11px] [&_.recharts-legend-item-text]:!text-[var(--fd-text-muted)] [&_.recharts-pie-label-text]:!text-[10px]">
      <div className="flex justify-between items-center px-5 py-3 border-[var(--fd-border)] border-b shrink-0">
        <h2 className="m-0 font-semibold text-[15px] text-[var(--fd-text)]">
          Script Statistics
        </h2>
        <button
          className="bg-transparent hover:bg-white/[0.06] px-1 py-0 border-none rounded text-[22px] text-[var(--fd-text-muted)] hover:text-[var(--fd-text)] leading-none cursor-pointer"
      ...
      <div className="flex flex-col flex-1 gap-6 p-5 overflow-y-auto">
          ...
          <div className="bg-[var(--fd-dropdown-bg)] p-4 border border-[var(--fd-border)] rounded-lg text-center">
            ...
            <div className="opacity-70 mt-0.5 text-[11px] text-[var(--fd-text-muted)]">
              ...
              {timingResult.totalSeconds > 0
                ...
                : formatTime(overview.estimatedRuntime)}
            </div>
          </div>
          <div className="bg-[var(--fd-dropdown-bg)] p-4 border border-[var(--fd-border)] rounded-lg text-center">
            <div className="font-bold text-[28px] text-[var(--fd-text)] leading-[1.2]">
              {overview.totalScenes}
            </div>
            <div className="mt-1 text-[11px] text-[var(--fd-text-muted)] uppercase tracking-[0.5px]">
              Scenes
            </div>
            <div className="opacity-70 mt-0.5 text-[11px] text-[var(--fd-text-muted)]">
              Avg {overview.averageSceneLength.toFixed(1)} pages
            </div>
          </div>
          <div className="bg-[var(--fd-dropdown-bg)] p-4 border border-[var(--fd-border)] rounded-lg text-center">
            <div className="font-bold text-[28px] text-[var(--fd-text)] leading-[1.2]">
              {overview.totalCharacters}
            </div>
            <div className="mt-1 text-[11px] text-[var(--fd-text-muted)] uppercase tracking-[0.5px]">
              Characters
            </div>
            <div className="opacity-70 mt-0.5 text-[11px] text-[var(--fd-text-muted)]">
              {overview.totalDialogueLines} dialogue lines
            </div>
          </div>
          <div className="bg-[var(--fd-dropdown-bg)] p-4 border border-[var(--fd-border)] rounded-lg text-center">
            <div className="font-bold text-[28px] text-[var(--fd-text)] leading-[1.2]">
        ...
        <div className="bg-[var(--fd-dropdown-bg)] p-4 border border-[var(--fd-border)] rounded-lg">
          <h3 className="m-0 mb-3 font-semibold text-[13px] text-[var(--fd-text)] uppercase tracking-[0.3px]">
          ...
          <div className="mb-3">
            {charDialogue.length > 0 ? (
              <ResponsiveContainer
                width="100%"
                height={Math.max(200, charDialogue.slice(0, 15).length * 28)}
              >
                <BarChart
                  data={charDialogue.slice(0, 15)}
                  layout="vertical"
          ...
          {charDialogue.length > 0 && (
            <table className="[&_tbody_tr:hover]:bg-white/[0.03] [&_td]:px-2 [&_th]:px-2 [&_td]:py-[5px] [&_th]:py-1.5 [&_td]:border-white/[0.04] [&_th]:border-[var(--fd-border)] [&_td]:border-b [&_th]:border-b w-full [&_th]:font-medium [&_td]:text-[var(--fd-text)] [&_th]:text-[10px] [&_th]:text-[var(--fd-text-muted)] text-xs [&_th]:text-left [&_th]:uppercase [&_th]:tracking-[0.3px] border-collapse">
              <thead>
                <tr>
                  <th>Character</th>
                  <th>Lines</th>
        ...
        <div className="bg-(--fd-dropdown-bg) border border-(--fd-border) rounded-lg p-4">
          <h3 className="text-[13px] font-semibold text-(--fd-text) m-0 mb-3 uppercase tracking-[0.3px]">
            Scene Breakdown
          </h3>
          <div className="gap-3 grid grid-cols-2 max-[768px]:grid-cols-1">
            ...
            <div className="bg-black/15 p-2.5 rounded-md">
              ...
              <ResponsiveContainer width="100%" height={160}>
                <BarChart
                  data={sceneBreakdown.sceneLengthBuckets}
            ...
            <div className="bg-black/15 p-2.5 rounded-md">
              ...
              {sceneBreakdown.locationFrequency.length > 0 ? (
                <ResponsiveContainer width="100%" height={160}>
                  <BarChart
                    data={sceneBreakdown.locationFrequency.slice(0, 8)}
                    layout="vertical"
        ...
        {charPresence.length > 0 && (
          <div className="bg-(--fd-dropdown-bg) border border-(--fd-border) rounded-lg p-4">
            ...
            <div className="-mx-4 px-4 overflow-x-auto">
              <table className="[&_td]:p-[3px] [&_th]:p-[3px] text-[11px] [&_td]:text-center [&_th]:text-center whitespace-nowrap border-collapse">
                <thead>
                  <tr>
                    <th className="!text-left !pr-3 sticky left-0 bg-(--fd-dropdown-bg) z-1 min-w-[100px] text-(--fd-text) font-medium">
                      Character
                    </th>
                    {pacingData.map((_, i) => (
                      <th
        ...
        {timingResult.scenes.length > 0 && (
          <div
            className="bg-(--fd-dropdown-bg) border border-(--fd-border) rounded-lg p-4"
            id="stats-timing-report"
          >
            <h3 className="text-[13px] font-semibold text-(--fd-text) m-0 mb-3 uppercase tracking-[0.3px]">
              Timing Report — Est. {formatRuntime(timingResult.totalSeconds)}
            </h3>
            <table className="w-full border-collapse text-xs [&_th]:text-left [&_th]:text-[10px] [&_th]:text-(--fd-text-muted) [&_th]:uppercase [&_th]:tracking-[0.3px] [&_th]:py-1.5 [&_th]:px-2 [&_th]:border-b [&_th]:border-(--fd-border) [&_th]:font-medium [&_td]:py-1.25 [&_td]:px-2 [&_td]:text-(--fd-text) [&_td]:border-b [&_td]:border-white/4 [&_tbody_tr:hover]:bg-white/3">
              ...
              <tbody>
                ...
                <tr
                  style={{
                    fontWeight: 700,
                    borderTop: '2px solid var(--fd-border)',
                  }}
                >
                  <td></td>
                  <td>TOTAL</td>
