...
const VersionHistory: React.FC = () => {
  ...
  const runScriptCompare = useCallback(async () => {
    if (!currentProject || !currentScriptId || compareSelection.length !== 2)
      return
    try {
      ...
      const aIsEarlier =
        new Date(vA.date).getTime() < new Date(vB.date).getTime()
      ...
      const fetchOrNull = async (hash: string) => {
        try {
          return await api.getScriptAtVersion(
            currentProject.id,
            hash,
            currentScriptId,
          )
  ...
  const handleViewDiff = useCallback(
    async (version: VersionInfo, index: number) => {
      ...
      try {
        ...
        setDiffText(
          `Error loading diff: ${err instanceof Error ? err.message : 'unknown error'}`,
        )
      }
    },
  ...
  const handleRestore = useCallback((version: VersionInfo) => {
    ...
    try {
      ...
      if (currentScriptId) {
        try {
          ...
          showToast(
            `Restored to version ${version.short_hash}. The open script no longer exists in this version.`,
            'info',
          )
          return
        }
  ...
  return (
    <div className="version-history-panel fixed top-0 right-0 w-[420px] h-screen bg-(--fd-navigator-bg) border-l border-(--fd-border) flex flex-col z-[3000] shadow-[-4px_0_20px_rgba(0,0,0,.4)]">
      <div className="flex justify-between items-center px-4 py-3 border-b border-(--fd-border) shrink-0">
        <span className="font-semibold text-[13px] uppercase tracking-[0.5px] text-(--fd-text)">
          Version History
        </span>
        <button
      ...
      {error && (
        <div className="px-4 py-5 text-[#ff6b6b] text-xs text-center italic">
          {error}
        </div>
      )}
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        <div className="flex-1 [&::-webkit-scrollbar-thumb]:bg-[#444] [&::-webkit-scrollbar-track]:bg-transparent py-2 [&::-webkit-scrollbar-thumb]:rounded-[3px] [&::-webkit-scrollbar]:w-1.5 overflow-y-auto version-history-list">
        ...
        {diffText !== null && selectedVersion && (
          <div className="border-t border-(--fd-border) max-h-1/2 flex flex-col shrink-0">
            <div className="flex justify-between items-center px-4 py-2 text-[11px] text-(--fd-text-muted) border-b border-(--fd-border) shrink-0">
              <span>
                Changes in {selectedVersion.short_hash}:{' '}
      ...
      {restoreConfirm && (
        <div
          className="top-0 right-0 left-0 z-[3000] fixed flex justify-center items-start bg-black/50 px-4 pt-[5vh] pb-4 h-[var(--vv-height,100dvh)] overflow-y-auto dialog-overlay"
