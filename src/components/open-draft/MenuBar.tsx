...
import { downloadOdraft, parseOdraft } from '@/utils/open-draft/odraftFormat'
...
import {
  useBackupStatusStore,
  describeBackupError,
} from '@/stores/backupStatusStore'
import RecoverBackupDialog from './RecoverBackupDialog'
import { openTextFile, openBinaryFile } from '@/utils/open-draft/fileOps'
...
const DiagRow: React.FC<{ label: string; value: string; mono?: boolean }> = ({
  label,
  value,
  mono,
}) => (
...
const MenuBar: React.FC<MenuBarProps> = ({
  ...
  const [formatPrefsOpen, setFormatPrefsOpen] = useState<{
    firstRun: boolean
    afterSave: 'apply-new-screenplay' | null
  } | null>(null)
  ...
  const [diagnosticsReport, setDiagnosticsReport] = useState<
    import('@/services/diagnostics').DiagnosticsReport | null
  >(null)
  ...
  const handleTrackChangesToggle = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (msg.includes('404')) {
          showToast(
            'This script has no checked-in version yet — use File > Check In first',
            'info',
          )
        } else {
          showToast(
            'Could not load the checked-in version. Try checking in first.',
            'error',
          )
        }
        return
      ...
      showToast(
        'Could not load version history. Make sure the backend is running.',
        'error',
      )
    }
  }, [
  ...
  useEffect(() => {
    if (activeMenu) {
      ...
      dropdownRef.current = document.body.querySelector(
        ':scope > .menu-dropdown',
      )
    } else {
  ...
  const [formatPickerMode, setFormatPickerMode] = useState<
    'reset' | 'apply-only'
  >('reset')
  ...
  const promptForNewScreenplayFormat = useCallback(
    (mode: 'reset' | 'apply-only') => {
      if (!editor) return
  ...
  const pendingFormatPromptInProject = useEditorStore(
    (s) => s.pendingFormatPromptInProject,
  )
  useEffect(() => {
  ...
  const handleBackupNow = useCallback(async () => {
    ...
    try {
      ...
      const reason = describeBackupError(
      ...
      items: [
          ...
          ? [
              {
                ...
                action: async () => {
                  try {
                    ...
                    showToast(
                      `Failed to open new window: ${err instanceof Error ? err.message : String(err)}`,
                      'error',
                    )
                  }
                },
        ...
        { separator: true, label: '' },
        {
          ...
          children: [
            {
              icon: <FaFileCode />,
              label: 'Final Draft / Fountain / OpenDraft...',
          ...
          children: [
            {
              ...
              action: handleExportFDX,
              disabled: isCollabGuest,
            },
            {
              icon: <FaFileAlt />,
              label: 'Fountain (.fountain)',
              action: handleExportFountain,
              disabled: isCollabGuest,
            },
        ...
        { separator: true, label: '' },
        {
          ...
          children: [
            ...
            },
            {
              ...
              action: () => setCompareVersionOpen(true),
            },
          ],
        },
              ...
              { separator: true, label: '' },
              {
                ...
                children: [
                  ...
                  },
                  {
                    ...
                    action: () => setRecoverBackupOpen(true),
                  },
                  { separator: true, label: '' },
                  {
                    icon: <FaFolderOpen />,
                    label: 'Open Backup Folder',
                    action: handleOpenBackupFolder,
                  },
                  {
                    icon: <FaCog />,
      ...
      items: [
        {
          ...
          children: [
            ...
            {
              icon: <FaUsers />,
              label: characterProfilesOpen ? '\u2713 Characters' : 'Characters',
          ...
          children: [
            ...
            },
            {
              ...
              action: () => {
                ...
                if (
                  localStorage.getItem('opendraft:hiddenModeIntroShown') !== '1'
                ) {
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          icon: <FaBoxes />,
          label: 'Asset Manager',
          action: () => useAssetStore.getState().toggleAssetManager(),
        },
        { separator: true, label: '' },
        {
          icon: <FaStream />,
        ...
        { separator: true, label: '' },
        {
          ...
          children: [
            {
              icon: <FaToggleOn />,
              label: revisionMode ? '\u2713 Revision Mode' : 'Revision Mode',
              action: () => setRevisionMode(!revisionMode),
            },
            { separator: true, label: '' },
            {
              icon: <FaListUl />,
  ...
  const helpMenu: MenuSection = {
    ...
    items: [
      ...
      },
      {
        ...
        action: () =>
          showToast(
            `${mod}1-8: Elements | Tab: Next | ${mod}B/I/U: Format | ${mod}Z: Undo | ${mod}F: Find | ${mod}G: Go to Page`,
            'success',
          ),
      },
      {
  ...
  const mapPluginChildren = (children: any[]): MenuItem[] =>
    children.map((c) => ({
      ...
      disabled:
        typeof c.disabled === 'function' ? c.disabled(pluginCtx) : c.disabled,
      separator: c.separator,
      children: c.children ? mapPluginChildren(c.children) : undefined,
    }))
  for (const menu of menus) {
    const pluginItems = pluginRegistry.getMenuItems(
      menu.label as PluginMenuSection,
    )
    if (pluginItems.length > 0) {
      ...
      for (const p of pluginItems) {
        menu.items.push({
          ...
          disabled:
            typeof p.disabled === 'function'
              ? p.disabled(pluginCtx)
              : p.disabled,
    ...
    (e: React.PointerEvent) => {
      ...
      if (d.isDrag) {
        setFabPos({
          x: Math.max(
            0,
            Math.min(window.innerWidth - FAB_SIZE, d.originX + dx),
          ),
          y: Math.max(
            0,
            Math.min(window.innerHeight - FAB_SIZE, d.originY + dy),
          ),
        })
      }
    },
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (menuRef.current && !menuRef.current.contains(target)) {
        ...
        if (target.closest('.menu-dropdown') || target.closest('.menu-fab'))
          return
        setFloatingMenuOpen(false)
      }
    }
  ...
  const menuBarClass =
    toolbarMode === 'comfortable' ? 'menu-bar chrome-comfortable' : 'menu-bar'
    ...
    <>
      {menus.map((menu) => (
        <div
          key={menu.label}
          ref={(el) => {
            menuItemRefs.current[menu.label] = el
          }}
        ...
        >
          {menuIcons[menu.label] && (
            <span className="menu-icon">{menuIcons[menu.label]}</span>
          )}
    ...
    <>
          ...
          <>
            ...
            {floatingMenuOpen &&
              (() => {
                ...
                return (
                  <div
                    className="menu-bar chrome-comfortable menu-bar--floating"
                    style={mStyle}
                    ref={menuRef}
                  >
                    {renderMenuItems()}
            ...
            {activeMenuData.items.map((item, i) =>
              item.separator ? (
                ...
                <div
                  ...
                  onPointerEnter={(e) =>
                    handleSubmenuPointerEnter(item.label!, e)
                  }
      ...
      {showHiddenModeIntro &&
        createPortal(
          <div
            ...
            onClick={() => setShowHiddenModeIntro(false)}
          >
            <div
              className="hidden-mode-intro"
            ...
            >
              ...
              <div className="hidden-mode-intro-body">
                ...
                <p>
                  The menu bar is now hidden. A{' '}
                  <strong>floating menu button</strong> has been placed on
                  screen to access all menus.
                </p>
                <p>
                  You can <strong>drag the button</strong> to reposition it
                  anywhere on screen. Your preferred position will be
                  remembered.
                </p>
                <p>
                  To restore the menu bar, tap the button and go to{' '}
                  <strong>View &gt; Menu &amp; Toolbar</strong>.
                </p>
              </div>
              <div
              ...
              >
                ...
                >
                  <input
                    type="checkbox"
                    checked={hiddenModeDontShow}
                    onChange={(e) => setHiddenModeDontShow(e.target.checked)}
                  />
                ...
                <button
                  ...
                  onClick={() => {
                    if (hiddenModeDontShow)
                      localStorage.setItem(
                        'opendraft:hiddenModeIntroShown',
                        '1',
                      )
            ...
            <div className="dialog-body">
              <div className="dialog-row">
                ...
                <input
                  ...
                  autoFocus
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && checkinMessage.trim())
                      handleCheckinSubmit()
        ...
        <ScriptFormatPreferencesDialog
          ...
          onConfirm={(ids) => {
            ...
            if (next?.afterSave === 'apply-new-screenplay') {
              ...
              if (ids.length === 1)
                finishNewScreenplayWithFormat(ids[0], formatPickerMode)
              else if (ids.length > 1) setFormatPickerOpen(true)
              else
                finishNewScreenplayWithFormat(
      ...
      {aboutOpen && (
        <div className="dialog-overlay" onClick={() => setAboutOpen(false)}>
          <div
            className="dialog-box about-dialog"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="dialog-body about-body">
              ...
              <div className="about-tagline">
                Free, open-source screenwriting software
              </div>
              ...
              <div className="about-whats-new">
                ...
                <div className="about-changelog">
                  ...
                  <ul className="about-list">
                    <li>
                      <strong>Automatic Backups</strong> — Choose a folder and
                      OpenDraft saves timestamped copies of your script there
                      while you write. Each project gets its own folder and the
                      first copy is written the moment you turn backups on. Back
                      up on demand with File → Backups → Back Up Now, and bring
                      any snapshot back with Recover Backup. Backups include
                      your notes, tags, beats, characters and images, and live
                      outside the app's database — so they survive even if
                      something happens to it. (Desktop only.)
                    </li>
                    <li>
                      <strong>Hard Line Breaks</strong> — Shift+Enter starts a
                      new line inside a screenplay element without creating a
                      new element. Breaks are preserved in PDF, Word, Final
                      Draft and Fountain exports, and counted correctly when
                      paginating.
                    </li>
                    <li>
                      <strong>More Reliable Saving</strong> — Manual saves no
                      longer drop per-script spelling and grammar settings, and
                      exported <code>.odraft</code> files now carry all of a
                      script's notes, tags, beats and character profiles.
                    </li>
                    <li>
                      <strong>Your Script Survives a Trip to Settings</strong> —
                      Opening Settings (or the project list) and coming back no
                      longer leaves the editor blank. Whatever you had open
                      returns exactly as you left it, including unsaved edits
                      and files opened straight from disk.
                    </li>
                  </ul>
                  ...
                  <ul className="about-list">
                    <li>
                      <strong>Mobile-Friendly Title Page</strong> — The Title
                      Page editor now adapts to small screens: the form and live
                      preview stack vertically and the dialog fits the viewport,
                      so there's no more horizontal scrolling on phones.
                    </li>
                  </ul>
                  ...
                  <ul className="about-list">
                    <li>
                      <strong>Insert Images</strong> — Add images anywhere in
                      the script and on the title page via Format → Insert
                      Image, paste from the clipboard, or drag &amp; drop.
                      Resize with the corner handle, set alignment, and they
                      export to PDF and Word.
                    </li>
                    <li>
                      <strong>Redesigned Title Page</strong> — Manage title-page
                      images (add, place top/bottom, align, remove), choose a
                      larger title font size, and preview the page live. The
                      editor, preview, and PDF/DOCX exports all match.
                    </li>
                    <li>
                      <strong>Mores &amp; Continueds</strong> — A new Format
                      dialog to control automatic character (CONT'D) and the
                      (MORE)/(CONT'D) at dialogue page breaks, with customizable
                      marker text.
                    </li>
                    <li>
                      <strong>Smarter (CONT'D)</strong> — (CONT'D) no longer
                      carries across a new scene, and deleting it at a specific
                      cue is remembered. Manually typed (CONT'D) is never
                      removed.
                    </li>
                    <li>
                      <strong>Larger Font Sizes</strong> — The font-size menus
                      now go up to 96pt.
                    </li>
                    <li>
                      <strong>Data-Loss Protection</strong> — A safeguard
                      prevents a blank/just-loaded editor from ever overwriting
                      a saved script with an empty document.
                    </li>
                  </ul>
                  ...
                  <ul className="about-list">
                    <li>
                      <strong>Edge-to-Edge Display on Android</strong> — Updated
                      how the editor handles modern Android screens so content
                      reaches the screen edges without sitting under the status
                      bar or gesture pill, addressing Play Console pre-launch
                      warnings.
                    </li>
                    <li>
                      <strong>Floating Menu Accessibility</strong> — The
                      hidden-menu icon now stays clear of the status bar so it's
                      tappable on every device.
                    </li>
                    <li>
                      <strong>Curved-Edge Layouts</strong> — Spelling, grammar,
                      search, and dialog panels now respect the safe area on
                      devices with curved displays so the panel never disappears
                      under the curve.
                    </li>
                    <li>
                      <strong>Reliable Spell-Check Markers</strong> — Replaced
                      the wavy underline on misspelled and grammar-flagged words
                      with a version that renders consistently across Android
                      WebView builds, including older OEM variants where the
                      native style was silently skipped.
                    </li>
                  </ul>
                  ...
                  <ul className="about-list">
                    <li>
                      <strong>Improved Stability</strong> — Resolves a startup
                      crash that affected the app on certain devices.
                    </li>
                    <li>
                      <strong>Per-Document Language &amp; Dictionary</strong> —
                      Choose a spellcheck language per script (English, Hindi,
                      Odia, and others) with a global custom-word library that
                  ...
                  <ul className="about-list">
                    <li>
                      <strong>Stable Caret on Format Changes</strong> — Toggling
                      bold, italic, underline, font, or size across a
                      multi-block selection no longer scrolls the viewport to
                      one end of the selection. The page stays put so you can
                  ...
                  <ul className="about-list">
                    <li>
                      <strong>Resend Verification Code</strong> — When signing
                      in on a new device with two-factor verification, you can
                      now request a fresh 6-digit code if the original email
                      didn't arrive.
                    </li>
                    <li>
                      <strong>Smarter Two-Factor Toggle</strong> — The Settings
                      two-factor switch is automatically disabled when the
                      collaboration server can't send email, so accounts can't
                      be accidentally locked out of new devices.
                    </li>
                  </ul>
                  ...
                  <ul className="about-list">
                    <li>
                      <strong>Save Prompt Before New / Open / Import</strong> —
                      The unsaved-changes dialog now also fires when auto-save
                      hasn't caught up yet. Edits made just before resetting the
                      editor are no longer silently discarded.
                    </li>
                    <li>
                      <strong>Faster Panel &amp; Search Navigation</strong> —
                      Clicking a character, scene, note, tag, or search match
                      now jumps the editor instantly instead of animating,
                      removing a noticeable lag on long paginated documents.
                    </li>
                  </ul>
                  ...
                  <ul className="about-list">
                    <li>
                      <strong>Save Reliability on Windows</strong> — Switched
                      the local SQLite database to WAL journal mode and added a
                      post-write byte-count verification step. Fixes silent save
                      failures on large files (issue #39). Any remaining write
                      corruption now produces a visible error instead of failing
                      silently.
                    </li>
                    <li>
                      <strong>OneDrive Detection</strong> — Warns you at startup
                      if OpenDraft's data folder is inside a OneDrive-synced
                      location (a known cause of silent SQLite corruption on
                      Windows) and shows how to fix it.
                    </li>
                    <li>
                      <strong>Diagnostics Dialog</strong> — New{' '}
                      <em>Help → Diagnostics</em> with a Copy Report button.
                      Captures storage backend, DB path, OS, and last storage
                      error so it can be pasted into bug reports.
                    </li>
                  </ul>
                  ...
                  <ul className="about-list">
                    <li>
                      <strong>Storage Fallback Recovery</strong> — If the app
                      falls back to in-memory storage after a SQLite failure, it
                      now recovers cleanly the next time SQLite becomes
                      available, and surfaces fallback errors instead of
                      swallowing them.
                    </li>
                    <li>
                      <strong>No More Lost Edits on Close</strong> — Pending
                      edits are flushed before the window closes, even on
                      unclean exits.
                    </li>
                    <li>
                      <strong>Mobile Stability</strong> — Dialogs survive the
                      soft keyboard on Android &amp; iOS; fixed an Android
                      cold-start crash.
                    </li>
                  </ul>
                  ...
                  <ul className="about-list">
                    <li>
                      <strong>Treatment Documents</strong> — Write a 20–25 page
                      prose treatment alongside your screenplay. Use "+ New
                      Document" in a project to open the manuscript-format
                      editor.
                    </li>
                    <li>
                      <strong>Location Database</strong> — Sidebar panel for
                      managing screenplay locations: list / detail / edit,
                      auto-discovery from scene headings, aliases, and
                      rename-in-scene-headings.
                    </li>
                    <li>
                      <strong>Act &amp; Sequence Structure</strong> — Tag scenes
                      into acts and sequences, browse them in a new Structure
                      tab in the Scene Navigator, with "A1"/"A2" badges on each
                      scene.
                    </li>
                    <li>
                      <strong>Version Diff View</strong> — Compare any two
                      checked-in versions side-by-side, unified, or
                      changes-only, with a summary of scenes changed and
                      per-character dialogue delta.
                    </li>
                    <li>
                      <strong>Multi-Format Templates</strong> — AV (two-column),
                      multicam sitcom, one-hour drama, radio play, and stage
                      play templates with a format picker for new screenplays.
                    </li>
                    <li>
                      <strong>DOCX Import / Export</strong> — Round-trip your
                      screenplay through Microsoft Word.
                    </li>
                    <li>
                      <strong>Title Page Editor</strong> — Structured editor
                      with live preview (Format &gt; Title Page); data flows
                      into PDF, FDX, and Fountain exports.
                    </li>
                    <li>
                      <strong>Script Statistics &amp; Timing</strong> — Tools
                      &gt; Analytics opens dialogue distribution, gender
                      analysis, pacing chart, and character presence map.
                      Per-scene timing in the Navigator and a runtime estimate
                      in the status bar.
                    </li>
                    <li>
                      <strong>WGA &amp; Registration Fields</strong> — Project
                      Properties gains WGA registration, copyright,
                      agent/manager fields, and a submission log.
                    </li>
                    <li>
                      <strong>Scene Navigator: Search &amp; Synopsis</strong> —
                      Search scene headings and synopses with highlighting;
                      inline synopsis preview on each collapsed scene.
                    </li>
                    <li>
                      <strong>Character Relationships</strong> — Inline
                      relationship editor, relationship map tab, and
                      profile-completeness indicator on the Characters panel.
                    </li>
                    <li>
                      <strong>Cloud Projects &amp; Per-User Files</strong> —
                      Configurable cloud server URL, per-user file isolation,
                      free 5-file quota, Local/Cloud project tabs, and
                      mobile-friendly tap targets.
                    </li>
                    <li>
                      <strong>Save As Replaces Save to Cloud</strong> —
                      Shift+Cmd+S now offers an explicit Local/Cloud destination
                      tab.
                    </li>
                    <li>
                      <strong>Self-Hosted Docker Image</strong> — Single{' '}
                      <code>ghcr.io/&hellip;/opendraft-combined</code> image
                      bundling backend + collab server for one-image deployment
                      targets.
                    </li>
                  </ul>
                </div>
          ...
          >
            ...
            <div className="dialog-body about-body">
              ...
              {diagnosticsReport ? (
                <>
                  ...
                  >
                    <tbody>
                      <DiagRow
                        label="Version"
                        value={diagnosticsReport.appVersion}
                      />
                      <DiagRow label="OS" value={diagnosticsReport.os} />
                      <DiagRow
                        label="Storage backend"
                        value={diagnosticsReport.storageMode}
                      />
                      {diagnosticsReport.storageError && (
                        <DiagRow
                          label="Storage error"
                          value={diagnosticsReport.storageError}
                          mono
                        />
                      )}
                      {diagnosticsReport.appDataDir && (
                        <DiagRow
                          label="App data dir"
                          value={diagnosticsReport.appDataDir}
                          mono
                        />
                      )}
                      {diagnosticsReport.sqliteDbPath && (
                        <DiagRow
                          ...
                          value={diagnosticsReport.sqliteDbPath}
                          mono
                        />
                      )}
                    </tbody>
                ...
                <div
                  style={{ marginTop: 12, color: 'var(--fd-text-secondary)' }}
                >
                  Loading…
                </div>
              )}
        ...
        <div className="dialog-overlay" onClick={handleDiscardConfirmCancel}>
          <div className="dialog-box" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="dialog-actions">
              ...
              <button
                className="dialog-primary"
                onClick={handleDiscardConfirmSave}
              >
                Save &amp; Continue
              </button>
            </div>
      ...
      {docxImportWarningOpen && (
        <div
          className="dialog-overlay"
          onClick={() => setDocxImportWarningOpen(false)}
        >
          <div className="dialog-box" onClick={(e) => e.stopPropagation()}>
            <div className="dialog-header">
              Import from Word — Best-Effort Formatting
            </div>
            <div className="dialog-body">
              <p
                style={{
              ...
              >
                OpenDraft will detect screenplay element types (scene heading,
                action, character, dialogue, parenthetical, transition, etc.)
                from the Word document&apos;s formatting.
              </p>
              <p
                style={{
                  margin: '0 0 8px 0',
              ...
              >
                Detection is <strong>best-effort</strong> and depends on
                consistent formatting being applied throughout the document.
              ...
              >
                <li>
                  Final Draft, Fade In, Trelby, or Highland style names, OR
                </li>
                <li>
                  Standard Final Draft indents (Action 1.5&quot;, Character
                  3.5&quot;, Dialogue 2.5&quot;, Parenthetical 3.0&quot;), OR
                </li>
                <li>
                  Conventional text patterns (INT./EXT., ALL-CAPS character
                  cues, &quot;CUT TO:&quot; transitions).
                </li>
              </ul>
              <p
                style={{
                  margin: 0,
                  fontSize: 13,
                  color: 'var(--fd-text-muted, #888)',
                }}
              >
                Anything that can&apos;t be classified will be imported as
                Action and listed in a post-import notice for you to review.
              </p>
            </div>
            <div className="dialog-actions">
              <button onClick={() => setDocxImportWarningOpen(false)}>
                Cancel
              </button>
              <button
                className="dialog-primary"
