...
const AddTargetToggle: React.FC<{
  active: boolean
  onToggle: () => void
  disabled?: boolean
...
const LanguagesSection: React.FC<{ onOpenInstaller: () => void }> = ({
  onOpenInstaller,
}) => {
...
const LanguageInstallerDialog: React.FC<{ onClose: () => void }> = ({
  onClose,
}) => {
  useSpellCheckerVersion()
  const installedLanguages = useEditorStore((s) => s.installedLanguages)
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className="flex-1 overflow-y-auto dialog-body"
          style={{
            display: 'flex',
            flexDirection: 'column',
            gap: 10,
        ...
        >
              ...
              return (
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div
                      style={{ fontSize: 11, color: 'var(--fd-text-muted)' }}
                    >
                      {lang.code} ·{' '}
          ...
          <div
            style={{ borderTop: '1px solid var(--fd-border)', paddingTop: 10 }}
          >
            <button
              type="button"
...
const GlobalDictionariesSection: React.FC<{ onOpenLibrary: () => void }> = ({
  onOpenLibrary,
}) => {
  useSpellCheckerVersion()
  ...
  return (
    <div style={cardStyle}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <button type="button" onClick={onOpenLibrary} style={buttonStyle}>
          Manage library…
        </button>
      </div>
      <div
...
const DictionaryConfigPanel: React.FC<{ onOpenLibrary: () => void }> = ({
  onOpenLibrary,
}) => {
  ...
  return (
    <>
      ...
      {installerOpen && (
        <LanguageInstallerDialog onClose={() => setInstallerOpen(false)} />
      )}
    </>
  )
}
