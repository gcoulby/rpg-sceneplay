...
const CTX_ITEM_DISABLED =
  ' text-(--fd-text-muted) cursor-default pointer-events-none'
const CTX_ITEM_ACTIVE =
  " before:content-['\\2713'] before:absolute before:left-[2px] before:text-[11px]"
...
const CTX_SHORTCUT =
  'text-[11px] text-(--fd-text-muted) ml-6 shrink-0 group-hover:text-white/60'
const CTX_ARROW =
  'text-[10px] text-(--fd-text-muted) ml-3 group-hover:text-white'
const CTX_SPELL_SUGGESTION =
  'font-semibold text-(--fd-accent) group-hover:text-white'
const CTX_COLOR_SWATCH =
  'inline-block w-3 h-3 rounded-[2px] mr-2 border border-white/15 shrink-0'
...
const ScriptContextMenu: React.FC<ScriptContextMenuProps> = ({
  ...
  const showDualDialogue = ['character', 'dialogue', 'parenthetical'].includes(
    currentNodeType,
  )
  ...
  const showCharProfile = ['character', 'dialogue', 'parenthetical'].includes(
    currentNodeType,
  )
  ...
  const existingTagInfo = (() => {
    ...
    return tagMark
      ? {
          tagId: tagMark.attrs.tagId as string,
          categoryId: tagMark.attrs.categoryId as string,
        }
      : null
  })()
  ...
  const handleSpellAddDictTo = useCallback(
    (target: string) => {
      if (!spellInfo) return
  ...
  return (
    ...
    >
      ...
      {grammarInfo && (
        <>
          <div
            className={`${CTX_ITEM}${CTX_ITEM_DISABLED}`}
          ...
          <div className={CTX_ITEM} onClick={handleGrammarIgnoreOnce}>
            Ignore Once
          </div>
          <div className={CTX_ITEM} onClick={handleGrammarIgnoreRuleForDoc}>
            Ignore in Document
          </div>
          <div className={CTX_ITEM} onClick={handleGrammarDisableRule}>
            Disable Rule
          </div>
          <div className={CTX_SEPARATOR} />
        </>
      ...
      {/* Clipboard */}
      <div
        className={`${CTX_ITEM}${!hasSelection ? CTX_ITEM_DISABLED : ''}`}
        onClick={handleCut}
      >
        <span>Cut</span>
      ...
      <div
        ...
        onPointerEnter={(e) => {
          if (e.pointerType === 'mouse') {
            setElementSubOpen(true)
            setStyleSubOpen(false)
            setRevisionSubOpen(false)
          }
        }}
        onPointerLeave={(e) => {
          if (e.pointerType === 'mouse') setElementSubOpen(false)
        }}
      >
        <div
          className={CTX_ITEM}
      ...
      <div
        ...
        onPointerEnter={(e) => {
          if (e.pointerType === 'mouse') {
            setStyleSubOpen(true)
            setElementSubOpen(false)
            setRevisionSubOpen(false)
          }
        }}
        onPointerLeave={(e) => {
          if (e.pointerType === 'mouse') setStyleSubOpen(false)
        }}
      >
        <div
          className={CTX_ITEM}
        ...
        {styleSubOpen && (
          <div className={CTX_SUBMENU}>
            <div
              className={`${CTX_ITEM}${locked.bold ? CTX_ITEM_DISABLED : ''}${editor.isActive('bold') ? CTX_ITEM_ACTIVE : ''}`}
              onClick={() => {
                if (!locked.bold) handleBold()
              }}
            >
              <span>Bold</span>
            ...
            <div
              className={`${CTX_ITEM}${locked.italic ? CTX_ITEM_DISABLED : ''}${editor.isActive('italic') ? CTX_ITEM_ACTIVE : ''}`}
              onClick={() => {
                if (!locked.italic) handleItalic()
              }}
            >
              <span>Italic</span>
            ...
            <div
              className={`${CTX_ITEM}${locked.underline ? CTX_ITEM_DISABLED : ''}${editor.isActive('underline') ? CTX_ITEM_ACTIVE : ''}`}
              onClick={() => {
                if (!locked.underline) handleUnderline()
              }}
            >
              <span>Underline</span>
            ...
            <div
              className={`${CTX_ITEM}${locked.strikethrough ? CTX_ITEM_DISABLED : ''}${editor.isActive('strike') ? CTX_ITEM_ACTIVE : ''}`}
              onClick={() => {
                if (!locked.strikethrough) handleStrike()
              }}
            >
            ...
            <div className={CTX_SEPARATOR} />
            <div
              className={`${CTX_ITEM}${locked.subscript ? CTX_ITEM_DISABLED : ''}${editor.isActive('subscript') ? CTX_ITEM_ACTIVE : ''}`}
              onClick={() => {
                if (!locked.subscript) handleSubscript()
              }}
            >
              <span>Subscript</span>
            </div>
            <div
              className={`${CTX_ITEM}${locked.superscript ? CTX_ITEM_DISABLED : ''}${editor.isActive('superscript') ? CTX_ITEM_ACTIVE : ''}`}
              onClick={() => {
                if (!locked.superscript) handleSuperscript()
              }}
            >
            ...
            <div className={CTX_SEPARATOR} />
            <div
              className={`${CTX_ITEM}${locked.textTransform ? CTX_ITEM_DISABLED : ''}`}
              onClick={() => {
                if (!locked.textTransform) handleAllCaps()
              }}
            >
              <span>ALL CAPS</span>
            </div>
      ...
      {/* Font & Formatting */}
      <div
        className={`${CTX_ITEM}${locked.fontFamily ? CTX_ITEM_DISABLED : ''}`}
        onClick={() => {
          if (!locked.fontFamily) {
            onOpenFormatPanel()
            onClose()
          }
        }}
      ...
      {showDualDialogue && (
        <div
          ...
          onClick={() => {
            console.log(
              '[CtxMenu] Dual Dialogue clicked, commands:',
              Object.keys(editor.commands).filter(
                (k) => k.includes('dual') || k.includes('Dual'),
              ),
            )
            const result = (editor as any).commands.toggleDualDialogue()
            console.log('[CtxMenu] result:', result)
            onClose()
          }}
        >
      ...
      {(showSceneProps || showDualDialogue || showCharProfile) && (
        <div className={CTX_SEPARATOR} />
      )}
      ...
      <div
        ...
        onPointerEnter={(e) => {
          if (e.pointerType === 'mouse') {
            setRevisionSubOpen(true)
            setElementSubOpen(false)
            setStyleSubOpen(false)
          }
        }}
        onPointerLeave={(e) => {
          if (e.pointerType === 'mouse') setRevisionSubOpen(false)
        }}
      >
        <div
          className={CTX_ITEM}
                ...
                {multipleTargets && addDictSubOpen && (
                  <div className={CTX_SUBMENU} style={{ left: '100%', top: 0 }}>
                    {activeAddTargets.map((t) => {
                      const label =
                        t === PROJECT_DICT_TARGET ? 'Project dictionary' : t
                      return (
                        <div
                          key={t}
                          className={CTX_ITEM}
